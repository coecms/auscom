...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_constants.F90 276 2010-05-05 21:49:36Z eclare $
!
...
!        rho_air   = 1.2_dbl_kind     ,&! ambient air density (kg/m^3)
      ...
      real (kind=dbl_kind) :: &  
         ...
         kice    ,&! thermal conductivity of fresh ice(W/m/deg)
         kseaice ,&! thermal conductivity of sea ice (W/m/deg)
                   ! (used in zero layer thermodynamics option)
...
#ifdef ACCICE
      real (kind=dbl_kind) :: &      ! currently used only
         awtvdr, &! visible, direct  ! for history and
         awtidr, &! near IR, direct  ! diagnostics
         awtvdf, &! visible, diffuse
         awtidf   ! near IR, diffuse
#else
