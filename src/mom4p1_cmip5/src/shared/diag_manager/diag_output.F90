  ...
  USE fms_mod, ONLY: lowercase, uppercase
    ...
    DO i = 1, num
       ...
       IF ( axis_cart_name == 'T' ) THEN
          ...
          tmp_cal = valid_calendar_types(calendar)
          if (trim(uppercase(tmp_cal)) == 'GREGORIAN') then
            tmp_cal="proleptic_gregorian"
          endif 
...
!          CALL mpp_write_meta(file_unit, id_time_axis, 'calendar', cval=TRIM(valid_calendar_types(calendar)))
          CALL mpp_write_meta(file_unit, id_time_axis, 'calendar_type', cval=TRIM(tmp_cal))
          CALL mpp_write_meta(file_unit, id_time_axis, 'calendar', cval=TRIM(tmp_cal))
