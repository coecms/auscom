/*
  ...
  This version reads the missing value for short, int, float and double variables.
  The output field is initialised with this value.
  This enhances the quality of output fields in the case, that land only
  tasks are masked.
  
  These changes are made by Franz Tauber, IOW. franz.tauber@io-warnemuende.de
  The code is not tested for all types of variables.
...
char missval[8];
short*shortmissval=(short*)&missval[0];
int*intmissval=(int*)&missval[0];
float*floatmissval=(float*)&missval[0];
double*doublemissval=(double*)&missval[0];
int missingvalueexists;
   ...
   if (!appendnc)
     {
      ...
      if ((ncoutfile.ncfid=nccreate(outfilename,NC_NOCLOBBER))==(-1))
        {
...
void usage()
  {
   printf("mppnccombine 2.0 - (written by Hans.Vahlenkamp@noaa.gov)\n\n");
...
int process_file(char *ncname, unsigned char appendnc,
                 ...
                 int *nrecs, int r, int f, void *varbuf[],
         ...
         for (n=0; n < ncinfile.natts[v]; n++)
           {
            ...
            /*-----------------------------------------------------*/
            missingvalueexists=0;
            if (strcmp(attname,"missing_value"))
            { ncattget(ncinfile.ncfid,v,"missing_value",(void *)missval);
              missingvalueexists=1;
            }
             /*-----------------------------------------------------*/
...
int process_vars(struct fileinfo *ncinfile, struct fileinfo *ncoutfile,
  ...
  {
   int v, d, i, j, k, l, b;  /* Loop variables */
   int dimid;  /* ID of a dimension */
   void *values;  /* Current data values */
   long instart[MAX_NC_DIMS], outstart[MAX_NC_DIMS];  /* Data array sizes */
   long count[MAX_NC_DIMS];                           /*        "         */
   long recsize;  /* Decomposed size of one record of a variable */
   long recfullsize;  /* Non-decomposed size of one record of a variable */
   ...
   for (v=0; v < ncinfile->nvars; v++)
     {
      ...
      if ((values=malloc(nctypelen(ncinfile->datatype[v])*recsize))==NULL)
        {
         fprintf(stderr,"Error: cannot allocate memory for decomposed variable \"%s\"'s values!\n",
                 ncinfile->varname[v]);
         return(1);
        }
         ...
         if (first)
           {
            ...
            if (verbose > 1)
              printf("        allocating %d bytes for full domain\n",
                     varbufsize);
            ...
            /*------------------------------------------------------------*/
            varbuf[v]=calloc(varbufsize,1);
/*             if (ncinfile->datatype[v]==NC_FLOAT)
            { float*x=varbuf[v];
              int imax= varbufsize/4;
              for (int i=0; i<imax; i++) x[i]=(*floatmissval); 
            }
 */
            ...
            {
            case NC_SHORT:
              { short*xshort=varbuf[v];
              int imax= varbufsize/2;
              for (i=0; i<imax; i++) xshort[i]=(*shortmissval); }
              break;
            case NC_INT:
              { int*xint=varbuf[v];
              int imax= varbufsize/4;
              for (i=0; i<imax; i++) xint[i]=(*intmissval);  }
              break;
            case NC_FLOAT:
              { float*xfloat=varbuf[v];
              int imax= varbufsize/4;
              for (i=0; i<imax; i++) xfloat[i]=(*floatmissval);  }
              break;
            case NC_DOUBLE:
              { double*xdouble=varbuf[v];
              int imax= varbufsize/8;
              for (i=0; i<imax; i++) xdouble[i]=(*doublemissval);  }
              break;
           ...
           /*------------------------------------------------------------*/
            ...
            if (varbuf[v]==NULL)
              {
               fprintf(stderr,"Error: cannot allocate memory for entire variable \"%s\"'s values!\n",
                       ncinfile->varname[v]); return(1);
              }
           }
         if (varbuf[v]==NULL)
           {
         ...
         if (verbose > 1)
           printf("        writing %ld bytes to memory\n",
                   nctypelen(ncinfile->datatype[v])*recsize);
