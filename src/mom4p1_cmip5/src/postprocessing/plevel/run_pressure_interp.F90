...
use  bgrid_halo_mod, only: update_halo, TEMP, UWND, VWND, EAST, NORTH, NOPOLE

use   constants_mod, only: constants_init, GRAV, RDGAS, RVGAS
use         fms_mod, only: fms_init, fms_end, error_mesg, FATAL, WARNING
...
!         rvort_name = relative vorticity
!           div_name = divergence
!--------------------------------------------------------
!
!   use_latlon = output momentum fields on the mass grid
!                 (the axis names lon & lat will be used)
!                
!  mask_extrap = set points below the bottom level to the missing value
!
!  unpack = if true, then data that was packed (e.g., int*2) will
!                be unpacked in the output file
!
...
!
!    3)  the following fields are always output:
!            temp, hght, slp, ucomp, vcomp
!
!    4)  specific humidity will be used to compute geop. height
!        only if the "sphum_name" namelist variable is set
!
!#######################################################################

namelist /names/ ucomp_name, vcomp_name, temp_name, sphum_name, &
                 rvort_name, div_name, hght_name, slp_name, &
                 ps_name, zsurf_name, eta_name, peta_name 

namelist /input/ in_file_name, out_file_name,  field_names,  pout, &
                 do_all_fields,  do_all_3d_fields,                 &
                 use_latlon, mask_extrap,                          &
...
logical    :: temp_present, sphum_present, winds_present

logical :: do_ucomp = .false.
logical :: do_vcomp = .false.
logical :: do_temp  = .false.
logical :: do_sphum = .false.
logical :: do_rvort = .false.
logical :: do_div   = .false.
logical :: do_zsurf = .false.
logical :: do_hght  = .false.
logical :: do_slp   = .false.

logical :: need_zsurf, need_temp, need_sphum, need_ucomp, need_vcomp

integer :: varid_res,      varid_pout
integer :: varid_eta,      varid_peta,     &
           varid_temp_in,  varid_sphum_in, &
           varid_zsurf_in, varid_ps_in,    &
           varid_ucomp_in, varid_vcomp_in

integer :: varid_hght_out, varid_slp_out, varid_rvort_out, varid_div_out
...
integer :: index_ps, index_zsurf, index_temp, index_sphum, index_ucomp, index_vcomp
...
integer :: is, ie, js, je
real    :: mval_hght, mval_slp, mval_rvort, mval_div
real(8) :: time, time_bnds(2)
logical :: done, do_phalf
integer :: idim, maxdimlen, nlen
real, allocatable :: axisdata(:)

real, allocatable, dimension(:)     :: eta, peta
real, allocatable, dimension(:,:)   :: res, pssl
...
!     ---- count the number of field names ----
  ...
  ! create version string (may replace with CVS $Id: run_pressure_interp.F90,v 1.1 2005/05/26 16:43:14 fms Exp $)
...
!-----------------  get input file information -------------------------
    ...
    !--- opening input files ---
     istat = NF90_OPEN (trim(in_file_name), NF90_NOWRITE, ncid_in)
...
!    --------- set flags for special computed fields --------
     ...
     do i = 1, num_field_names
        ...
        if (field_names(i) == rvort_name) then
            do_rvort = .true.; cycle; endif
        if (field_names(i) ==   div_name) then
            do_div   = .true.; cycle; endif
...
!--- get the size of the vertical axis ---
     istat = NF90_INQUIRE_VARIABLE (ncid_in, varid_eta, ndims=ndim, dimids=dimids)
     if (istat /= NF90_NOERR) call error_handler ('getting dimids for '//trim(eta_name), ncode=istat)
     istat = NF90_INQUIRE_DIMENSION (ncid_in, dimids(1), len=nlev)
     if (istat /= NF90_NOERR) call error_handler ('getting dim length for '//trim(eta_name), ncode=istat)
     nlev = nlev-1
...
!--- set flags for special non-computed fields ---
     ...
     do i = 1, num_field_names
        ...
        if (field_names(i) == ucomp_name) then
            do_ucomp = .true.; cycle; endif
        if (field_names(i) == vcomp_name) then
            do_vcomp = .true.; cycle; endif
        if (field_names(i) == zsurf_name) then
            do_zsurf = .true.; cycle; endif
     enddo

!--------------------------------------------------------------
!------- get name and size of input momentum axes -------

     if (winds_present) then
        istat = NF90_INQUIRE_VARIABLE (ncid_in, varid_ucomp_in, &
                                       ndims=ndim, dimids=dimids)
        if (istat /= NF90_NOERR) call error_handler (ncode=istat)

        if (ndim /= 4) call error_handler &
         ('variable '//trim(temp_name)//' must have dimensions x,y,z,t')

       !--- size and name of input horizontal momentum axes ---
        istat = NF90_INQUIRE_DIMENSION (ncid_in, dimids(1), name=xmom_in) !, len=nlon)
        if (istat /= NF90_NOERR) call error_handler ('getting name of first momentum dimension', ncode=istat)
        istat = NF90_INQUIRE_DIMENSION (ncid_in, dimids(2), name=ymom_in) !, len=nlat)
        if (istat /= NF90_NOERR) call error_handler ('getting name of second momentum dimension', ncode=istat)

       !---- A-grid check ----
        if (trim(xmom_in) == trim(xmass_in) .and. &
            trim(ymom_in) == trim(ymass_in)) winds_present = .false.
     else
        xmom_in = xmass_in
        ymom_in = ymass_in
     endif
...
!-----------------------------------------------------------------------
     ...
     call horiz_grid_init ( Hgrid, nlon, nlat )   !, layout=
     is = Hgrid%Tmp%is;  ie = Hgrid%Tmp%ie
     js = Hgrid%Tmp%js;  je = Hgrid%Tmp%je

!    ---- allocate space ----

     allocate (eta(nlev+1), peta(nlev+1))
     allocate (res  (is:ie,js:je),    &
               zsurf(is:ie,js:je,1),  &
               ps   (is:ie,js:je,1),  &
               pssl (Hgrid%ilb:Hgrid%iub,Hgrid%jlb:Hgrid%jub))

!    ---- read vert coord variables and set up vert coord ----

     istat = NF90_GET_VAR (ncid_in, varid_eta, eta)
     if (istat /= NF90_NOERR) call error_handler (ncode=istat)
     if (varid_peta > 0) then
        istat = NF90_GET_VAR (ncid_in, varid_peta, peta)
        if (istat /= NF90_NOERR) call error_handler (ncode=istat)
     else
        peta = 0.0
     endif
...
!----------------- output file header info -----------------------------
   ...
   ! create file
     istat = NF90_CREATE (trim(out_file_name), NF90_CLOBBER, ncid_out)
     if (istat /= NF90_NOERR) call error_handler (ncode=istat)
...
!------------------ generate axes for output file ----------------------
     ...
     if (use_latlon) then
        xmass_out = 'lon';  ymass_out = 'lat'
        xmom_out  = 'lon';  ymom_out  = 'lat'
     else
        xmass_out = xmass_in;  ymass_out = ymass_in
        xmom_out  = xmom_in;   ymom_out  = ymom_in
     endif

    !---- copy mass axes ----
     naxes = 1
     call copy_axis_information (ncid_in, xmass_in, ncid_out, Axes(naxes),     &
                                 dimid_out=dimids_mass(1), name_out=xmass_out, &
                                 name_edges=edge_name)
         if (edge_name(1:1) .ne. ' ') then
             naxes = naxes + 1
             call copy_axis_information (ncid_in, edge_name, ncid_out, Axes(naxes))
         endif
     naxes = naxes + 1
     call copy_axis_information (ncid_in, ymass_in, ncid_out, Axes(naxes),     &
                                 dimid_out=dimids_mass(2), name_out=ymass_out, &
                                 name_edges=edge_name)
         if (edge_name(1:1) .ne. ' ') then
             naxes = naxes + 1
             call copy_axis_information (ncid_in, edge_name, ncid_out, Axes(naxes))
    ...
    !---- copy momentum axes when necessary ----
     if (.not.use_latlon) then
         naxes = naxes + 1
         call copy_axis_information (ncid_in, xmom_in, ncid_out, Axes(naxes),    &
                                     dimid_out=dimids_mom(1), name_out=xmom_out, &
                                     name_edges=edge_name)
             if (edge_name(1:1) .ne. ' ') then
                 naxes = naxes + 1
                 call copy_axis_information (ncid_in, edge_name, ncid_out, Axes(naxes))
         ...
         naxes = naxes + 1
         call copy_axis_information (ncid_in, ymom_in, ncid_out, Axes(naxes),    &
                                     dimid_out=dimids_mom(2), name_out=ymom_out, &
                                     name_edges=edge_name)
             if (edge_name(1:1) .ne. ' ') then
                 naxes = naxes + 1
                 call copy_axis_information (ncid_in, edge_name, ncid_out, Axes(naxes))
         ...
         dimids_mom(1:2) = dimids_mass(1:2)
     endif
...
!    -------- vertical axis (convert to mb) -------
   ...
   ! momentum always uses the same vertical and time axes
     dimids_mom(3:4) = dimids_mass(3:4)
...
if (do_rvort .and. winds_present) then
   mval_rvort = NF90_FILL_REAL4
   call define_new_variable ( ncid_out, rvort_name, NF90_REAL4, dimids_mass,        &
                              varid_rvort_out, units='1/s',                         &
                              long_name='relative vorticity',                       &
                              missing_value=mval_rvort, time_avg_info=time_avg_info,&
                              cell_methods=cell_methods )
endif

!   ---- divergence ----

if (do_div .and. winds_present) then
   mval_div = NF90_FILL_REAL4
   call define_new_variable ( ncid_out, div_name, NF90_REAL4, dimids_mass,        &
                              varid_div_out, units='1/s',                         &
                              long_name='divergence',                             &
                              missing_value=mval_div, time_avg_info=time_avg_info,&
                              cell_methods=cell_methods)
endif
...
!---- copy variable info for other fields ------
    ...
    do i = 1, num_field_names
       ...
       if (trim(field_names(i)) == trim(ucomp_name)) index_ucomp = i
       if (trim(field_names(i)) == trim(vcomp_name)) index_vcomp = i
       ...
       if (trim(field_names(i)) == trim(rvort_name)) cycle
       if (trim(field_names(i)) == trim(  div_name)) cycle
...
!---- check to see if these variables are in the field_names list ----
      ...
      if (need_ucomp) then
          if (.not.field_name_present(ucomp_name,field_names,num_field_names)) then
              num_field_names = num_field_names + 1
              index_ucomp = num_field_names
              call get_input_variable_info ( ucomp_name, ncid_in, Fields(num_field_names) )
          endif
      endif

      if (need_vcomp) then
          if (.not.field_name_present(vcomp_name,field_names,num_field_names)) then
              num_field_names = num_field_names + 1
              index_vcomp = num_field_names
              call get_input_variable_info ( vcomp_name, ncid_in, Fields(num_field_names) )
          endif
      endif
...
!------------ read and write static axis data -------------
       ...
       maxdimlen = maxval(Axes(1:naxes)%dimlen)
       allocate (axisdata(maxdimlen))

       do idim = 1, naxes
          if (Axes(idim)%varid_in < 0) cycle
          nlen = Axes(idim)%dimlen
          istat = NF90_GET_VAR (ncid_in, Axes(idim)%varid_in, axisdata(1:nlen))
          if (istat /= NF90_NOERR) call error_handler ('getting axis data for var='//trim(Axes(idim)%name_in), ncode=istat)
          istat = NF90_PUT_VAR (ncid_out, Axes(idim)%varid_out, axisdata(1:nlen))
...
!------------ first time: read constant fields -------------------------
       ...
       if ( istat == NF90_NOERR ) then
          istat = NF90_GET_VAR (ncid_in, varid_res, res)
          if (istat /= NF90_NOERR) call error_handler (ncode=istat)
       else
          res = 1.0
       endif
       if (need_zsurf .or. do_zsurf) then
          call read_variable (ncid_in, Fields(index_zsurf), zsurf)
       else
          zsurf = 0.0
       endif
       if (do_zsurf) then
...
!      ---- read/write static fields ----
    ...
    do i = 1, num_field_names
       ...
       call read_write_static (ncid_in, ncid_out, is, js, Fields(i))
    enddo
...
!-------------- interpolation setup ------------------------------------
     ...
     !---- surface pressure ----
          ...
          pssl(is:ie,js:je) = ps(:,:,1) * res
          call update_halo ( Hgrid, TEMP, pssl )
...
!     ---- interpolation initialization for mass fields ------
     ...
     call interp_field_setup ( ncid_in, ncid_out, Vgrid,    &
                               is, js, itime, otime,        &
                               pssl(is:ie,js:je),           &
...
!----------------- velocity field interpolation ---------------------
    ...
    if (winds_present) then
      call interp_vel_field ( ncid_in, ncid_out, Hgrid, Vgrid,      &
                              pssl, pout(1:nlev_out), itime, otime, &
                              Fields(index_ucomp),                  &
                              Fields(index_vcomp)                   )
    ! prevent redundant processing of winds
      Fields(index_ucomp)%skip = .true.
      Fields(index_vcomp)%skip = .true.
    endif

!  ---- interpolation of fields on velocity grid (not efficient) ----

     do i = 1, num_field_names
        if ( Fields(i)%skip   )    cycle
        if ( Fields(i)%static )    cycle
        if ( Fields(i)%y_axis /= nlat-1 ) cycle
...
!#######################################################################
   ...
   subroutine interp_field_setup ( ncid_in, ncid_out, Vgrid,   &
                                   is, js, itime, otime, pssl, &
                                   zsurf, pout )

   integer               , intent(in) :: ncid_in, ncid_out
   type  (vert_grid_type), intent(in) :: Vgrid
                  integer, intent(in) :: is, js, itime, otime
                  real   , intent(in) :: pssl(:,:), zsurf(:,:), pout(:)

 real, dimension(size(pssl,1),size(pssl,2),Vgrid%nlev) :: &
            temp_in, sphum_in, mask_in, pfull, hght_in
 real, dimension(size(pssl,1),size(pssl,2),Vgrid%nlev+1)   :: zhalf, &
                                                              phalf
 real, dimension(size(pssl,1),size(pssl,2),size(pout)) ::  &
                         temp_out, hght_out, sphum_out
 real   , dimension(size(pssl,1),size(pssl,2)) :: zbot, tbot, pbot, slp
 integer, dimension(size(pssl,1),size(pssl,2)) :: kbot
   character(len=NF90_MAX_NAME) :: name
   integer :: start(4), kount(4), ndim, ivar, nlev, kb, kr, i, j, k
   real    :: sig


      call compute_pres_full (Vgrid, pssl, pfull)
  ...
  !---- interpolation setup for standard full pressure levels -----
   if (do_hght) then
      !---- geopotential height (in meters) -----
       call compute_height ( Vgrid, zsurf*GRAV, temp_in, sphum_in, &
                             pfull, phalf, hght_in, zhalf, mask_in )
...
!        ---- sea level pressure ----
   ...
   if (do_slp) then
     ...
     do i = 1, size(pssl,1)
       ...
       !if ( abs(zhalf(i,j,kb+1)) > 0.0001 ) then
...
!     ---- check horiz axis lengths ----
       ...
       if ( Field%dimlen(1) /= Interp%nx .or. Field%dimlen(2) /= Interp%ny ) &
           call error_handler (   &
             'horizontal coordinate resolution inconsistent')
...
!#######################################################################
 ...
 subroutine interp_vel_field (ncid_in, ncid_out, Hgrid, Vgrid, &
                              pssl, pout, itime, otime,        &
                              Field_ucomp, Field_vcomp         )

               integer , intent(in)    :: ncid_in, ncid_out
 type (horiz_grid_type), intent(inout) :: Hgrid
 type  (vert_grid_type), intent(in)    :: Vgrid
                real   , intent(in)    :: pssl(Hgrid%ilb:,Hgrid%jlb:), &
                                          pout(:)
                integer, intent(in)    :: itime, otime
       type(field_type), intent(in)           :: Field_ucomp
       type(field_type), intent(in), optional :: Field_vcomp

 real, dimension(Hgrid%ilb:Hgrid%iub,    &
                 Hgrid%jlb:Hgrid%jub) :: pssl_xy

 real, dimension(Hgrid%ilb:Hgrid%iub,              &
                 Hgrid%jlb:Hgrid%jub,Vgrid%nlev) :: u, v, pfull, mask_in

 real, dimension(Hgrid%ilb:Hgrid%iub,              &
                 Hgrid%jlb:Hgrid%jub,size(pout)) :: up, vp, vort, div, &
                                                    upp, vpp

 real, dimension(Hgrid%ilb:Hgrid%iub,              &
                 Hgrid%jlb:Hgrid%jub,size(pout)) :: rmask_out, rmask_one

 logical, dimension(Hgrid%ilb:Hgrid%iub,              &
                    Hgrid%jlb:Hgrid%jub,size(pout)) :: mask_out

 integer, dimension(Hgrid%ilb:Hgrid%iub,    &
                    Hgrid%jlb:Hgrid%jub) :: kbot

  integer :: start(4), kount(4), nd, ivar, nlev, k
  integer :: is, ie, vs, ve, hs, he, js, je
  integer :: isd, ied,jsd, jed
  logical :: use_wind_components, use_ucomp, use_vcomp
  logical :: do_miss
  real    :: missval

!-----------------------------------------------------------------------

      if (present(Field_vcomp)) then
         use_wind_components = .true.
         use_ucomp = need_ucomp .or. do_ucomp
         use_vcomp = need_vcomp .or. do_vcomp
      else
         use_wind_components = .false.  ! processing a single field
         use_ucomp = .true.
         use_vcomp = .false.
      endif

    ! do not need to process any momentum fields
      if (.not.use_ucomp .and. .not.use_vcomp) return
...
!     ---- compute pressure at full levels ----
      ...
      call compute_pres_full (Vgrid, pssl, pfull  )
      call change_grid (Hgrid, TEMP_GRID, WIND_GRID, pfull, pfull)
      call update_halo (Hgrid, UWND, pfull, halos=EAST+NORTH, flags=NOPOLE)
...
!   ---- read u-comp -----
    ...
    if (use_ucomp) then
       call read_variable (ncid_in, Field_ucomp, u(is:ie,vs:ve,:), start)
       call update_halo (Hgrid, UWND, u)
       do_miss = Field_ucomp%do_miss_in
       missval = Field_ucomp%unpkd_missval_in
    endif

!   ---- read v-comp (assume t-ave info same as u-comp) -----

    if (use_vcomp) then
       call read_variable (ncid_in, Field_vcomp, v(is:ie,vs:ve,:), start)
       call update_halo (Hgrid, VWND, v)
       if (.not.use_ucomp) then
           u(isd:ied,jsd:jed,:) = v(isd:ied,jsd:jed,:)
           do_miss = Field_vcomp%do_miss_in
           missval = Field_vcomp%unpkd_missval_in
       endif
    else
       v(isd:ied,jsd:jed,:) = u(isd:ied,jsd:jed,:)
    endif


  ! set mask for missing value if it exists (use u-comp value only)
         kbot = nlev
      if (do_miss) then
         where ( u(isd:ied,jsd:jed,:) == missval )
            mask_in(isd:ied,jsd:jed,:) = 0.0
         elsewhere
            mask_in(isd:ied,jsd:jed,:) = 1.0
         endwhere

         do k = nlev, 1, -1
            if (count(mask_in(isd:ied,jsd:jed,k) < 0.5) == 0) exit
            where (mask_in(isd:ied,jsd:jed,k) < 0.5) kbot(isd:ied,jsd:jed) = k-1
         enddo
      endif

!     ---- interpolation (data domain only) ----

    call wind_interp ( pfull(isd:ied,jsd:jed,:), &
                       u(isd:ied,jsd:jed,:), v(isd:ied,jsd:jed,:), &
                       pout, kbot(isd:ied,jsd:jed), &
                       up(isd:ied,jsd:jed,:), vp(isd:ied,jsd:jed,:), &
                       mask_out(isd:ied,jsd:jed,:) )

!     ---- set-up real output data mask and temporary masked data ----
...
!---- compute new fields from masked momemtum fields ----- 

    if (use_wind_components) then
       if (do_rvort) call compute_vorticity  ( Hgrid, upp, vpp, vort )
       if (do_div)   call compute_divergence ( Hgrid, upp, vpp, div )
    endif

!---- interpolate data to mass grid (do not mask data) ----

    rmask_one = 1.0

    if (use_latlon) then
       call change_grid (Hgrid, WIND_GRID, TEMP_GRID, up, vp, up, vp, &
                         mask_inp=rmask_one)
!      ---- re-compute mask at mass points ----
       call change_grid (Hgrid, WIND_GRID, TEMP_GRID, rmask_out, rmask_out, &
                         mask_inp=rmask_one)
       mask_out = rmask_out < 0.05
    endif
...
!---- output velocity components ----
    ...
    if (do_ucomp) then
        if (Field_ucomp%do_miss_out) &
        call mask_data (Field_ucomp%unpkd_missval_out, mask_out, up)
        call write_variable (ncid_out, Field_ucomp, up(is:ie,js:je,:), start)
    endif

   !--- skip rest of code if single field ---
    if (.not.use_wind_components) return

    if (do_vcomp) then
        if (Field_vcomp%do_miss_out) &
        call mask_data (Field_vcomp%unpkd_missval_out, mask_out, vp)
        call write_variable (ncid_out, Field_vcomp, vp(is:ie,js:je,:), start)
    endif

!---- output vorticity and divergence ----- 

!   ---- re-compute mask at mass points (if necessary) ----
    if (do_rvort .or. do_div) then
       if ( .not. use_latlon) then
          call change_grid (Hgrid, WIND_GRID, TEMP_GRID, rmask_out, rmask_out, &
                            mask_inp=rmask_one)
          mask_out = rmask_out < 0.05
    ...
    if (do_rvort) then
        start = (/ is, hs, 1, otime /)
        call mask_data (mval_rvort, mask_out, vort)
        istat = NF90_PUT_VAR (ncid_out, varid_rvort_out, vort(is:ie,js:je,:), start)
        if (istat /= NF90_NOERR) call error_handler ('putting rvort data', ncode=istat)
    endif

    if (do_div) then
        start = (/ is, hs, 1, otime /)
        call mask_data (mval_div, mask_out, div)
        istat = NF90_PUT_VAR (ncid_out, varid_div_out, div(is:ie,js:je,:), start)
        if (istat /= NF90_NOERR) call error_handler ('putting div data', ncode=istat)
    endif

   end subroutine interp_vel_field
...
!#######################################################################
  ...
  subroutine compute_vorticity (Hgrid, u, v, vort)

  type (horiz_grid_type), intent(in) :: Hgrid
  real, intent(in),  dimension(Hgrid%ilb:,Hgrid%jlb:,:) :: u, v
  real, intent(out), dimension(Hgrid%ilb:,Hgrid%jlb:,:) :: vort

  real, dimension(Hgrid%ilb:Hgrid%iub, &
                  Hgrid%jlb:Hgrid%jub) :: vdy,  udx, avdy, audx
  integer :: i, j, k, is, ie, js, je

!
!    computes relative vorticity on constant pressure levels
!      input momentum fields are on the momentum grid and
!      the output vorticity field is on the mass grid
!
   ...
   do k = 1, size(u,3)
      ...
      do j = js-1, je+1
         vdy(:,j) = v(:,j,k) * Hgrid%Vel%dy
         udx(:,j) = u(:,j,k) * Hgrid%Vel%dx(j)
      enddo
...
!     ------ vorticity ------
      ...
      do j = js, je
      do i = is, ie
         vort(i,j,k) = ((avdy(i,j)-avdy(i-1,j)) -  &
                        (audx(i,j)-audx(i,j-1)))*Hgrid%Tmp%rarea(j)*0.50
      enddo
      enddo
...
!#######################################################################
  ...
  subroutine compute_divergence (Hgrid, u, v, div)
  ...
  real, dimension(Hgrid%ilb:Hgrid%iub, &
                  Hgrid%jlb:Hgrid%jub) :: udy,  vdx, few, fns
  integer :: i, j, k, is, ie, js, je

!
!    computes divergence on constant pressure levels
!      input momentum fields are on the momentum grid and
!      the output divergence field is on the mass grid
!
   ...
   do k = 1, size(u,3)
      ...
      do j = js-1, je+1
         udy(:,j) = u(:,j,k) * Hgrid%Vel%dy
         vdx(:,j) = v(:,j,k) * Hgrid%Vel%dx(j)
      enddo

      do j = js,   je
      do i = is-1, ie
         few(i,j) = udy(i,j) + udy(i,j-1)
      enddo
      enddo
      do j = js-1, je
      do i = is,   ie
         fns(i,j) = vdx(i,j) + vdx(i-1,j)
      enddo
      enddo
...
!#######################################################################
 ...
 subroutine check_fields ( ncid )
   ...
   integer :: id(8), ierr, iwarn, i
   integer :: avg(8), istat(8)
   character(len=NF90_MAX_NAME), dimension(8) :: names

!-----------------------------------------------------------------------

     names = (/ eta_name, peta_name, zsurf_name, ps_name, &
                temp_name, sphum_name, ucomp_name, ucomp_name /)
...
!------ perform checks ------
   ...
   ! set variable identifiers in the input file
      ...
      if ( ierr  /= 0 ) call error_mesg ('run_pressure_interp', ' ', FATAL)
      if ( iwarn /= 0 ) call error_mesg ('run_pressure_interp', ' ', WARNING )
             
    ! momentum components (set flag if both present)
      if (istat(7) == NF90_NOERR .and. istat(8) == NF90_NOERR) then
          winds_present = .true.
          avg(7) = NF90_INQUIRE_ATTRIBUTE (ncid, id(7), 'time_avg_info')
          avg(8) = NF90_INQUIRE_ATTRIBUTE (ncid, id(8), 'time_avg_info')
      else
          winds_present = .false.
      endif
    ...
    ! relative vorticity and divergence
      ...
      !  if (.not.do_ucomp) winds_present = .false.
      endif
...
!     ---- axes must have correct size ----
        if (axlen(1) /= nlon .or.  (axlen(2) /= nlat .and. &
                                    axlen(2) /= nlat-1))   cycle
        if (threed) then
...
!  ------ skip fields that are written separately ------
       ...
       !if (.not.winds_present) return

        if (trim(field_name) == trim(rvort_name)) then
            skip_field_name = .true.; return; endif
        if (trim(field_name) == trim(div_name)) then
            skip_field_name = .true.; return; endif

       !if (.not.winds_present) return

       !if (trim(field_name) == trim(ucomp_name)) then
       !    skip_field_name = .true.; return; endif
       !if (trim(field_name) == trim(vcomp_name)) then
       !    skip_field_name = .true.; return; endif
...
!#######################################################################
 ...
 ! skip certain field names
   ...
   if (trim(name) == trim(rvort_name)) return
   if (trim(name) == trim(div_name))   return
...
!#######################################################################
 ...
 subroutine copy_variable_information ( name, ncid_in, ncid_out, Field )
   ...
   !     allow momentum grid interpolation (nlat-1)
     if (Field%ndim >= 2) then
         if (Field%dimlen(1) /= nlon .or. &
            (Field%dimlen(2) /= nlat .and. Field%dimlen(2) /= nlat-1)) then
   ...
   ! set dimension identifiers for output file
     ...
     if (Field%dimlen(2) == nlat-1) dimids = dimids_mom
...
!#######################################################################
 ...
 subroutine copy_axis_information (ncid_in, name_in, ncid_out, Axis, &
                                   dimid_out, name_out, name_edges)
 ...
 integer :: varid_in, varid_out, dimlen, dim_out

   if (verbose > 2) print *, 'COPYING, name_in=',trim(name_in)
   call copy_axis ( ncid_in, name_in, ncid_out, dim_out, varid_in, varid_out, &
                    dimlen, name_out=name_out, name_edges=name_edges)
   ...
   if (present(dimid_out)) dimid_out = dim_out
...
!#######################################################################
 ...
 subroutine read_and_set_up_mask (ncid, start, mask, kbot)
   ...
   character(len=NF90_MAX_NAME) :: name
   real,  dimension(nlon,nlat,nlev) :: values
...
!#######################################################################
 ...
 subroutine read_write_static (ncid_in, ncid_out, is, js, Field)
   integer         , intent(in) :: ncid_in, ncid_out, is, js
   type(field_type), intent(in) :: Field

   real :: values(nlon,nlat,1)
 ...
 ! write data
   call write_variable ( ncid_out, Field, values, start=(/is, js/))
