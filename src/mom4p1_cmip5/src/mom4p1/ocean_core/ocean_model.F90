...
!XXXXX determine 'do_sfix_now' here or in ocean_solo.F90!
    ...
    do num_ocn = 1,num_ocean_calls
       ...
       call get_ocean_sbc(Time, Ice_ocean_boundary, Thickness, Ext_mode, T_prog(1:num_prog_tracers), &
            Velocity, pme, melt, river, runoff, calving, upme, uriver, swflx, swflx_vis, patm, aice)
#else
       call get_ocean_sbc(Time, Ice_ocean_boundary, Thickness, Ext_mode, T_prog(1:num_prog_tracers), &
...
#ifdef AusCOM
       call flux_adjust(Time, T_diag(1:num_diag_tracers), T_prog(1:num_prog_tracers), Velocity, river, melt, pme, aice)
#else
