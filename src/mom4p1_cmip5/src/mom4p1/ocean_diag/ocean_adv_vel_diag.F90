...
character(len=128) :: version=&
     '$Id: ocean_adv_vel_diag.F90,v 16.0.2.2.48.3.22.1.4.1.14.1 2009/12/14 15:04:59 smg Exp $'
...
namelist /ocean_adv_vel_diag_nml/ max_cfl_value, large_cfl_value, &
                                  verbose_cfl, diag_step
...
id_tz_trans_sq = register_diag_field ('ocean_model','tz_trans_sq', Grd%tracer_axes_wt(1:3),&
              ...
              missing_value=missing_value, range=(/-1e21,1e21/),                             &
...
id_tx_trans_rho = register_diag_field ('ocean_model','tx_trans_rho', Dens%potrho_axes(1:3),   &
                  Time%model_time, 'T-cell i-mass transport on pot_rho',trim(transport_dims), &
                  missing_value=missing_value, range=(/-1e20,1e20/))
id_ty_trans_rho = register_diag_field ('ocean_model','ty_trans_rho', Dens%potrho_axes(1:3),   &
                  Time%model_time, 'T-cell j-mass transport on pot_rho',trim(transport_dims), &
                  missing_value=missing_value, range=(/-1e20,1e20/))

id_tx_trans_nrho = register_diag_field ('ocean_model','tx_trans_nrho', Dens%neutralrho_axes(1:3), &
                   Time%model_time, 'T-cell i-mass transport on neutral rho',trim(transport_dims),&
                   missing_value=missing_value, range=(/-1e20,1e20/))
id_ty_trans_nrho = register_diag_field ('ocean_model','ty_trans_nrho', Dens%neutralrho_axes(1:3), &
                   Time%model_time, 'T-cell j-mass transport on neutral rho',trim(transport_dims),&
                   missing_value=missing_value, range=(/-1e20,1e20/))

id_tx_trans_theta = register_diag_field ('ocean_model','tx_trans_theta', Dens%theta_axes(1:3), &
                    Time%model_time, 'T-cell i-mass transport on theta',trim(transport_dims),  &
                    missing_value=missing_value, range=(/-1e20,1e20/))
id_ty_trans_theta = register_diag_field ('ocean_model','ty_trans_theta', Dens%theta_axes(1:3), &
...
subroutine transport_on_nrho (Time, Dens, Adv_vel)
  ...
  if (need_data(id_tx_trans_nrho,next_time) .or. need_data(id_ty_trans_nrho,next_time)) then
         ...
         do j=jsc,jec
            do i=isc,iec
               if(Dens%neutralrho(i,j,k) <= Dens%neutralrho_ref(1)) then 
                   work(i,j,1,1) = work(i,j,1,1) + Adv_vel%uhrho_et(i,j,k)
                   work(i,j,1,2) = work(i,j,1,2) + Adv_vel%vhrho_nt(i,j,k)
               endif
            enddo
         enddo
      enddo
         ...
         do j=jsc,jec
            do i=isc,iec
               if(Dens%neutralrho_ref(neutralrho_nk) < Dens%neutralrho(i,j,k)) then 
                   work(i,j,neutralrho_nk,1) = work(i,j,neutralrho_nk,1) + Adv_vel%uhrho_et(i,j,k)             
                   work(i,j,neutralrho_nk,2) = work(i,j,neutralrho_nk,2) + Adv_vel%vhrho_nt(i,j,k)
               endif
            enddo
         enddo
      enddo
               ...
               do i=isc,iec
                  if(     Dens%neutralrho_ref(k_rho) <  Dens%neutralrho(i,j,k)  ) then
                      if( Dens%neutralrho(i,j,k)     <= Dens%neutralrho_ref(k_rho+1)) then 
                          ...
                          work(i,j,k_rho,1)   = work(i,j,k_rho,1)   + weight*Adv_vel%uhrho_et(i,j,k)
                          work(i,j,k_rho,2)   = work(i,j,k_rho,2)   + weight*Adv_vel%vhrho_nt(i,j,k)
                          work(i,j,k_rho+1,1) = work(i,j,k_rho+1,1) + (1.0-weight)*Adv_vel%uhrho_et(i,j,k)
                          work(i,j,k_rho+1,2) = work(i,j,k_rho+1,2) + (1.0-weight)*Adv_vel%vhrho_nt(i,j,k)
                      endif
                  endif
               enddo
            enddo
      ...
      do k_rho=1,neutralrho_nk
         do j=jsc,jec
            do i=isc,iec
               work(i,j,k_rho,1) = work(i,j,k_rho,1)*Grd%dyte(i,j)*transport_convert*Grd%tmask(i,j,1)
               work(i,j,k_rho,2) = work(i,j,k_rho,2)*Grd%dxtn(i,j)*transport_convert*Grd%tmask(i,j,1)
            enddo
         enddo
      enddo

      if (id_tx_trans_nrho > 0) then 
          used = send_data (id_tx_trans_nrho, work(:,:,:,1), Time%model_time, &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
      if (id_ty_trans_nrho > 0) then 
          used = send_data (id_ty_trans_nrho, work(:,:,:,2), Time%model_time, &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=neutralrho_nk)
      endif
...
subroutine transport_on_rho (Time, Dens, Adv_vel)
  ...
  if (need_data(id_tx_trans_rho,next_time) .or. need_data(id_ty_trans_rho,next_time)) then
         ...
         do j=jsc,jec
            do i=isc,iec
               ...
               if(Dens%potrho_ref(potrho_nk) < Dens%potrho(i,j,k)) then 
                   work(i,j,potrho_nk,1) = work(i,j,potrho_nk,1) + Adv_vel%uhrho_et(i,j,k)             
                   work(i,j,potrho_nk,2) = work(i,j,potrho_nk,2) + Adv_vel%vhrho_nt(i,j,k)
               endif
            enddo
         enddo
      enddo
               ...
               do i=isc,iec
                  if(     Dens%potrho_ref(k_rho) <  Dens%potrho(i,j,k)  ) then
                      if( Dens%potrho(i,j,k)     <= Dens%potrho_ref(k_rho+1)) then 
                          weight = (Dens%potrho(i,j,k)-Dens%potrho_ref(k_rho)) &
                                  /(Dens%potrho_ref(k_rho+1)-Dens%potrho_ref(k_rho))
                          work(i,j,k_rho,1)   = work(i,j,k_rho,1)   + weight*Adv_vel%uhrho_et(i,j,k)
                          work(i,j,k_rho,2)   = work(i,j,k_rho,2)   + weight*Adv_vel%vhrho_nt(i,j,k)
                          work(i,j,k_rho+1,1) = work(i,j,k_rho+1,1) + (1.0-weight)*Adv_vel%uhrho_et(i,j,k)
                          work(i,j,k_rho+1,2) = work(i,j,k_rho+1,2) + (1.0-weight)*Adv_vel%vhrho_nt(i,j,k)
      ...
      if (id_tx_trans_rho > 0) then 
          used = send_data (id_tx_trans_rho, work(:,:,:,1), Time%model_time, &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=potrho_nk)
      endif
      if (id_ty_trans_rho > 0) then 
          used = send_data (id_ty_trans_rho, work(:,:,:,2), Time%model_time, &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=potrho_nk)
      endif
...
subroutine transport_on_theta (Time, Dens, Theta, Adv_vel)
  ...
  if (need_data(id_tx_trans_theta,next_time) .or. need_data(id_ty_trans_theta,next_time)) then
         ...
         do j=jsc,jec
            do i=isc,iec
               if(Dens%theta_ref(theta_nk) < Theta%field(i,j,k,tau)) then 
                   work(i,j,theta_nk,1) = work(i,j,theta_nk,1) + Adv_vel%uhrho_et(i,j,k)
                   work(i,j,theta_nk,2) = work(i,j,theta_nk,2) + Adv_vel%vhrho_nt(i,j,k)
               endif
            enddo
         enddo
      enddo
               ...
               do i=isc,iec
                  if(     Dens%theta_ref(k_theta) <  Theta%field(i,j,k,tau) ) then
                      if( Theta%field(i,j,k,tau)  <= Dens%theta_ref(k_theta+1)) then 
                          ...
                          weight = (Theta%field(i,j,k,tau)   -Dens%theta_ref(k_theta)) &
                                  /(Dens%theta_ref(k_theta+1)-Dens%theta_ref(k_theta))
                          work(i,j,k_theta,1)   = work(i,j,k_theta,1)   + weight*Adv_vel%uhrho_et(i,j,k)
                          work(i,j,k_theta,2)   = work(i,j,k_theta,2)   + weight*Adv_vel%vhrho_nt(i,j,k)
                          work(i,j,k_theta+1,1) = work(i,j,k_theta+1,1) + (1.0-weight)*Adv_vel%uhrho_et(i,j,k)
                          work(i,j,k_theta+1,2) = work(i,j,k_theta+1,2) + (1.0-weight)*Adv_vel%vhrho_nt(i,j,k)
                      endif
                  endif
               enddo
            enddo
      ...
      do k_theta=1,theta_nk
         do j=jsc,jec
            do i=isc,iec
               work(i,j,k_theta,1) = work(i,j,k_theta,1)*Grd%dyte(i,j)*transport_convert*Grd%tmask(i,j,1)
               work(i,j,k_theta,2) = work(i,j,k_theta,2)*Grd%dxtn(i,j)*transport_convert*Grd%tmask(i,j,1)
            enddo
         enddo
      enddo

      if (id_tx_trans_theta > 0) then 
          used = send_data (id_tx_trans_theta, work(:,:,:,1), Time%model_time, &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=theta_nk)
      endif
      if (id_ty_trans_theta > 0) then 
          used = send_data (id_ty_trans_theta, work(:,:,:,2), Time%model_time, &
                 is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=theta_nk)
      endif
...
! friction visc_cbu and vertical velocity wrho_bu.
! </DESCRIPTION>
!
    ...
    do j=jsc,jec
      do i=isc,iec
        ...
        if (k >= Grd%kmu(i,j)) then
          ...
          reyz = Grd%umask(i,j,kk)*abs(Adv_vel%wrho_bu(i,j,k)*Thickness%dzwu(i,j,k))*ramb
        endif
        if (reyz > reynz) then
          ireynz = i
          jreynz = j
  ...
  if (reynz == reynz0) then
     write(unit,'(/,a,es9.2,a,i4,a1,i4,a1,i3,a,f7.2,a,f7.2,a,f8.2,a,/,1x,a,es9.2,a,es9.2,/)') &
           ...
           Grd%yu(ireynz,jreynz),',', Grd%dzt(kreynz),' m). ', 'Vertical velocity "w_bu" is (m/s) ',&
            rho0r*reynw, ' and vertical diabatic viscosity (m^2/s) "visc_cbu" is ', reynmw
  endif
