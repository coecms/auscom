...
!  </DATA> 
!  </DATA> 
!  <DATA NAME="deflate" TYPE="logical">
!  Gently reduce restoring near end
!  </DATA> 
!  <DATA NAME="deflate_fraction" TYPE="logical">
!  Start deflating after this fraction of the restoring period has passed.
!  </DATA> 
...
subroutine sponge_eta_source(Time, Ext_mode)
  ...
  if ( first_pass ) num_surface_wet_cells = mpp_global_sum(Dom%domain2d,Grd%tmask(:,:,1))
  ...
  if (Sponge(1)%id > 0) then
     ...
     if ( do_adaptive_restore ) then
        if ( first_pass ) then
           ...
           wrk2_2d=abs(Ext_mode%eta_t(:,:,taum1)-wrk1_2d(:,:))*Grd%tmask(:,:,1)
