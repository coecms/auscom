...
! mom4p0 code (available in ocean_vert_kpp_mom4p0.F90) incorrectly
...
! </INFO>
!
!<NAMELIST NAME="ocean_vert_kpp_nml">
!
...
!  implementation has kl_min=2.  Default in mom4p1 is kl_min=2. 
!  </DATA> 
...
use constants_mod,    only: grav, epsln
...
use ocean_parameters_mod, only: cp_ocean, missing_value, von_karman 
...
integer                            :: id_hblt                    =-1
integer                            :: id_neutral_rho_kpp_nonlocal=-1
integer                            :: id_wdian_rho_kpp_nonlocal  =-1
integer                            :: id_ws                      =-1
logical                            :: used
...
character(len=256) :: version=&
     '$Id: ocean_vert_kpp.F90,v 1.1.2.2.2.20.6.2.16.1.16.2.20.1 2009/10/10 00:42:34 nnz Exp $'
...
namelist /ocean_vert_kpp_nml/ use_this_module, shear_instability, double_diffusion, &
...
!#######################################################################
! <SUBROUTINE NAME="ocean_vert_kpp_init">
!
! <DESCRIPTION>
! Initialization for the KPP vertical mixing scheme
! </DESCRIPTION>
!
subroutine ocean_vert_kpp_init (Grid, Domain, Time, Time_steps, T_prog, T_diag, &
  ...
  if ( module_is_initialized ) then 
     call mpp_error(FATAL, &
      '==>Error from ocean_vert_kpp_mod (ocean_vert_kpp_init): module is already initialized')
  endif 
  ...
  read  (ioun, ocean_vert_kpp_nml,iostat=io_status)
  write (stdoutunit,'(/)')
  write (stdoutunit, ocean_vert_kpp_nml)  
  write (stdlogunit, ocean_vert_kpp_nml)
  ierr = check_nml_error(io_status,'ocean_vert_kpp_nml')
  ...
  if(vert_coordinate==ZSIGMA .or. vert_coordinate==PSIGMA) then 
    ...
    '==>ocean_vert_kpp_mod: ZSIGMA or PSIGMA are not fully working w/ KPP. ')
    call mpp_error(WARNING,&
    'Problems exist /w tracer balances not closing AND instabilities w/ nonlocal transport.')
  endif  

  write(stdoutunit,'(/a,f10.2)') &
  '==>Note from ocean_vert_kpp_mod: using forward time step for vert-frict of (secs)', &
                               Time_steps%dtime_u 
  write(stdoutunit,'(/a,f10.2)') &
  '==>Note from ocean_vert_kpp_mod: using forward time step for vert-diff  of (secs)', &
                               Time_steps%dtime_t 
  if(kbl_standard_method) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: adjust kbl to hbl with the standard method.'
  else
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: adjust kbl to hbl with the non-standard method.'
    if (kl_min > 1) write(stdoutunit,'(1x,a,I4)') '==>WARNING from ocean_vert_kpp_mod:'// &
                    ' change kl_min to 1 to avoid negative mixing coefficients with low wind stress.'
  endif
  if(use_sbl_bottom_flux) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: '// &
  ...
  if (linear_hbl) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Use linear interpolation to find hbl'
  else
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Use quadratic interpolation to find hbl'
  endif
  if(limit_ghats) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Limit ghats*diff_cbt to 1.'
  else
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Do not limit ghats*diff_cbt to 1.'
  endif
  if(limit_with_hekman) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Limit hbl with hekman for stable case.'
  else
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Do not limit hbl with hekman for stable case.'
  endif
  if(calc_visc_on_cgrid) then  
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Calculate viscosity at c-grid.'
  else
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Interpolate viscosity to c-grid.'
  endif
  if(radiation_large) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: sw-radiation in non-local scheme h_gamma=h.'
  elseif (radiation_zero) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: sw-radiation in non-local scheme with h_gamma=0.'
  elseif (radiation_iow) then
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: sw-radiation in non-local scheme with IOW-method.'
  else
    write(stdoutunit,'(1x,a)') '==> NOTE from ocean_vert_kpp_mod: Leave full sw-radiation in non-local surface flux.'
  endif
  
  if(radiation_large .and. radiation_zero) call mpp_error(FATAL,&
      '==>ocean_vert_kpp_mod: Do not enable radiation_large and radiation_zero together. ')   
  if(radiation_large .and. radiation_iow) call mpp_error(FATAL,&
      '==>ocean_vert_kpp_mod: Do not enable radiation_large and radiation_iow together. ')   
  if(radiation_zero .and.  radiation_iow) call mpp_error(FATAL,&
      '==>ocean_vert_kpp_mod: Do not enable radiation_zero and radiation_iow together. ')   
  ...
  if (index_temp < 1 .or. index_salt < 1) then 
    call mpp_error(FATAL, &
    '==>Error: ocean_vert_kpp_mod: temp and/or salt not present in tracer array')
  endif 
...
!-----------------------------------------------------------------------
  ...
  if (Time_steps%aidif /= 1.0) then
    call mpp_error(FATAL,&
    '==>Error in ocean_vert_kpp_mod (ocean_vert_kpp_init): KPP must use aidif=1 for implicit vertical mixing')
  endif
  ...
  id_neutral_rho_kpp_nonlocal = register_diag_field ('ocean_model', &
          'neutral_rho_kpp_nonlocal',                               &
          Grid%tracer_axes(1:3), Time%model_time,                   &
          'neutral density tendency due to KPP nonlocal term',      &
          'rho*rho_dz/sec',missing_value=missing_value, range=(/-1e10,1e10/))
  id_wdian_rho_kpp_nonlocal = register_diag_field ('ocean_model',          &
          'wdian_rho_kpp_nonlocal', Grd%tracer_axes(1:3), Time%model_time, &
          'dianeutral velocity component due to KPP nonlocal term',        &
...
end subroutine ocean_vert_kpp_init
! </SUBROUTINE> NAME="ocean_vert_kpp_init">



!#######################################################################
! <SUBROUTINE NAME="vert_mix_kpp">
!
! <DESCRIPTION>
...
! </DESCRIPTION>
!
subroutine vert_mix_kpp (aidif, Time, Thickness, Velocity, T_prog, T_diag, Dens, &
...
!-----------------------------------------------------------------------
  ...
  call ri_for_kpp(Time, Thickness, aidif, Velocity, T_prog, &
                  Dens%pressure_at_depth, Dens%rho(:,:,:,tau), Dens%rho(:,:,:,taum1))
...
!-----------------------------------------------------------------------
        ...
        dbloc1(:,:,:) = density_delta_z(Dens%rho(:,:,:,tau), T_prog(index_salt)%field(:,:,:,tau),   &
                        T_prog(index_temp)%field(:,:,:,tau), Dens%pressure_at_depth(:,:,:))
        dbsfc1(:,:,:) = density_delta_sfc(Dens%rho(:,:,:,tau), T_prog(index_salt)%field(:,:,:,tau), &
                        T_prog(index_temp)%field(:,:,:,tau), Dens%pressure_at_depth(:,:,:))
        do k=2,nk
           do j=jsd,jed
              do i=isd,ied
...
!         these are proper units for buoyancy fluxes. 
          ...
          smftu = rho0r*(Velocity%smf(i,j,1)   + Velocity%smf(i-1,j,1)     &
                        +Velocity%smf(i,j-1,1) + Velocity%smf(i-1,j-1,1))  &
                  /active_cells
          smftv = rho0r*(Velocity%smf(i,j,2)   + Velocity%smf(i-1,j,2)    &
                        +Velocity%smf(i,j-1,2) + Velocity%smf(i-1,j-1,2)) &
                   /active_cells
...
!     instability.
!-----------------------------------------------------------------------
      call ri_iwmix(Time, Thickness, Dens%rho(:,:,:,tau), visc_cbu, diff_cbt)
...
!-----------------------------------------------------------------------
      ...
      if (id_ws > 0) then 
        used = send_data(id_ws, wrk1(:,:,:), Time%model_time, rmask=Grd%tmask(:,:,:),  &
...
!-----------------------------------------------------------------------
       ...
       if (non_local_kpp) then
           ...
           do n=1,num_prog_tracers
              ...
              if(use_sbl_bottom_flux) then
                  do j=jsc,jec
                     ...
                     enddo
                  enddo
                  if(id_wbot(n) > 0) used = send_data(id_wbot(n), wrk2(:,:,1), &
              ...
              if (n==index_temp .or. n==index_salt) then
                  if(id_ghats(ni) > 0) then 
                      if (limit_ghats) then
                          used = send_data(id_ghats(ni), min(ghats(:,:,:)*diff_cbt(:,:,:,ni),1.), &
                               Time%model_time, rmask=Grd%tmask(:,:,:),  			  &
                               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
                      else
                          used = send_data(id_ghats(n), ghats(:,:,:)*diff_cbt(:,:,:,ni), &
                               Time%model_time, rmask=Grd%tmask(:,:,:),  		 &
                               is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
                      endif
                  endif
              ...
              if (id_nonlocal(n) > 0) then 
                  used = send_data(id_nonlocal(n),T_prog(n)%conversion*T_prog(n)%wrk1(:,:,:), &
                       Time%model_time, rmask=Grd%tmask(:,:,:),                               &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
              endif
           ...
           if(id_neutral_rho_kpp_nonlocal > 0) then
               wrk1(:,:,:) = 0.0
               do k=1,nk
                  do j=jsc,jec
                     do i=isc,iec
                  ...
                  enddo
               enddo
               used = send_data(id_neutral_rho_kpp_nonlocal, wrk1(:,:,:), &
                      Time%model_time, rmask=Grd%tmask(:,:,:),            &
                      is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)

           endif
           if(id_wdian_rho_kpp_nonlocal > 0) then
               wrk1(:,:,:) = 0.0
               do k=1,nk
                  do j=jsc,jec
                     do i=isc,iec
                  ...
                  enddo
               enddo
               used = send_data(id_wdian_rho_kpp_nonlocal, wrk1(:,:,:), &
                      Time%model_time, rmask=Grd%tmask(:,:,:),          &
                      is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
           endif
...
! </DESCRIPTION>
!
subroutine ri_iwmix(Time, Thickness, rho, visc_cbu, diff_cbt)
