...
!
! This code is more general than that in the ocean_vert_kpp_mod.
! The KPP code remains part of mom4p1 for legacy purposes. 
!</DESCRIPTION>
!
! <INFO>
!
...
!  in the absence of a roughness length dataset. mom4p1 default
...
!  </DATA> 
!
!  <DATA NAME="compute_bvfreq_legacy" TYPE="logical">
!  To compute bvfreq using an older algorithm.  
!  There are good reasons to prefer the newer approach, which motivates 
!  setting the default compute_bvfreq_legacy=.false.
!  </DATA> 
...
use constants_mod,     only: pi, epsln, grav
...
use ocean_parameters_mod, only: von_karman, rho0, rho0r, omega_earth
...
!use auscom_ice_parameters_mod, only: lat_low_bgdiff, bg_diff_eq   !20110802 
!use mpp_domains_mod, only: mpp_global_field, mpp_get_global_domain    !for 'debug' purpose
use mpp_mod, only: mpp_pe    !temporary use
implicit none
...
real, private :: pipi = 3.1415926                        !temporary use

!integer, private :: isg,ieg,jsg,jeg           !for 'debug' purpose      
!real, dimension (:,:), allocatable :: vowrk   !for 'debug' purpose 
!
...
character (len=128) :: tagname = '$Name: mom4p1_riga_smg $'
...
logical :: compute_bvfreq_legacy       = .false.  
...
namelist /ocean_vert_tidal_nml/ use_this_module, debug_this_module,                             &
                                ...
                                smooth_bvfreq_bottom, vel_micom_smooth, compute_bvfreq_legacy,  & 
...
! </DESCRIPTION>
  subroutine ocean_vert_tidal_init(Grid, Domain, Time, T_prog, Ocean_options, dtime_t, vert_mix_scheme)
...
!---------------------------------------------------------------------------------------------------------
! ars599+dhb599 20110802: calculation of the latitude-dependent bg_diff
...
! background_diffusivity is used for regions beyond the equatorial zone (10S-10N).
! Between 10N and 10S the background diffusivity is latitude-dependent and calculated here 
! using function designed by Arnold Sullivan. 
! Ref: Jochum, M., 2009: Impact of latitudinal variations in vertical diffusivity on climate simulations.
!
! The following algorithm features the bg_diff as a function of latitude within the predefined equatorial band:
...
! 
    ...
    coef_a = (bgdiff_max - bgdiff_min)/(cos(lat_low_bgdiff*pipi/180.) - 1.) 
    coef_b = bg_diff_eq - coef_a
    do j=jsd,jed
      do i=isd,ied
        bg_diff(i,j) = background_diffusivity 
        if ( abs(Grd%yt(i,j)).le.lat_low_bgdiff) then
          !bg_diff(i,j) = (-cos((Grd%yt(i,j))/2/asin(1))+1)*(background_diffusivity-bg_diff_eq) + bg_diff_eq
          bg_diff(i,j) = coef_a * cos(Grd%yt(i,j)*pipi/180.) + coef_b
        endif
       enddo
    enddo
!!!temporary check:
    do i = isd, ied
       if (i .eq. 1) then
         do j = jsd, jed
           write(111 + mpp_pe(),'(i3,4x,2e14.6)') j, Grd%yt(i,j), bg_diff(i,j)
         enddo
       endif
    enddo
!!!

!for debug purpose (not complete yet!):
!    call mpp_get_global_domain(Dom,isg,ieg,jsg,jeg)
!    allocate (vwork(isg:ieg, jsg:jeg)); vwork = 0.0
! 
!---------------------------------------------------------------------------------------------------------
...
! </DESCRIPTION>
!
  subroutine vert_mix_tidal(Time, Thickness, T_prog, Dens, diff_cbt, visc_cbu, &
  ...
  if(use_wave_dissipation)  then 
      call vert_mix_wave(Time, Thickness, Dens, diff_cbt, visc_cbu, diff_cbt_wave, diff_cbt_leewave)
  endif
  if(use_drag_dissipation)  then
      call vert_mix_drag(Time, Thickness, diff_cbt, visc_cbu, diff_cbt_drag)
  endif
...
!
! Maintain a legacy approach, which is not recommended.
!
! </DESCRIPTION>
!
      ...
      do k=1,nk
         ...
         do j=jsd,jed
            do i=isd,ied
               ...
               wrk2(i,j,k) = tmp*(T_prog(index_salt)%field(i,j,k,tau)-T_prog(index_salt)%field(i,j,kp1,tau)) 
            enddo
         enddo
      enddo
  ...
  else  ! not compute_bvfreq_legacy
  ...
  endif ! endif for compute_bvfreq_legacy
  ...
  if(.not. compute_bvfreq_legacy) then 
      ...
      ! replace bvfreq in bottom roughness boundary 
      ! layer with the maximum bvfreq within that layer.
      ! define max to be no less than bvfreq(kbot-1)
      do j=jsd,jed
         do i=isd,ied
            kbot=Grd%kmt(i,j)
            if(kbot > 1) then 
                tmp = bvfreq(i,j,kbot-1)
                do k=roughness_klevel(i,j),kbot 
                   tmp = max(tmp,bvfreq(i,j,k))
                enddo
                do k=roughness_klevel(i,j),kbot 
                   bvfreq(i,j,k) = tmp 
                enddo
                bvfreq_bottom(i,j) = tmp
            endif
         enddo
      enddo
...
!
! We assume a unit Prandtl number, so compute viscosity as a four-point
! average of diffusivity. 
!
...
! </DESCRIPTION>
!
  subroutine vert_mix_wave(Time, Thickness, Dens, diff_cbt, visc_cbu, diff_cbt_wave, diff_cbt_leewave)
...
! </DESCRIPTION>
!
  subroutine vert_mix_drag(Time, Thickness, diff_cbt, visc_cbu, diff_cbt_drag)
