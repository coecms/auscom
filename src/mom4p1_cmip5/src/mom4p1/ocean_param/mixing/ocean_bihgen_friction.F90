...
logical :: equal_coefficients    = .FALSE. ! Equal k_smag and vel_micom. Can optimise.
...
subroutine ocean_bihgen_friction_init(Grid, Domain, Time, Ocean_options, d_time, &
  ...
  equal_coefficients = ( k_smag_iso == k_smag_aniso ) .and. ( vel_micom_iso == vel_micom_aniso )
  if ( equal_coefficients ) then
      write( stdoutunit,'(a)')' Isotropic and anisotropic coefficients are equal. Optimizing calculation'
  endif
...
subroutine bihgen_friction(Time, Thickness, Velocity, bih_viscosity, energy_analysis_step)
  ...
  do k=1,nk
     ...
     if ( equal_coefficients ) then  ! Standard code
     do j=jsc,jec
        do i=isc,iec
     ...
     do j=jsc,jec
        do i=isc,iec
          ...
          ! dimensions[stress]=m^2/s^1.5
           ...
           aiso_smag(i,j,ip,jq)    = min(visc_crit(i,j), aiso_back(i,j,k)   + fsmag_iso(i,j)*deform)
           aiso_smag(i,j,ip,jq)    = sqrt(aiso_smag(i,j,ip,jq))
           aaniso_smag(i,j,ip,jq)= aiso_smag(i,j,ip,jq)
!           stress(i,j,ip,jq,1) = aiso_smag(i,j,ip,jq)*(tension + delta*sin2theta(i,j))
!           stress(i,j,ip,jq,2) = aiso_smag(i,j,ip,jq)*(strain  - delta*cos2theta(i,j))
           stress(i,j,ip,jq,1) = aiso_smag(i,j,ip,jq)*tension + aaniso_smag(i,j,ip,jq)*delta*sin2theta(i,j)
           stress(i,j,ip,jq,2) = aiso_smag(i,j,ip,jq)*strain  - aaniso_smag(i,j,ip,jq)*delta*cos2theta(i,j)
