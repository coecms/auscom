      SUBROUTINE locrint (cdfldn, kfield, kdimax, knulre, kflgre)
C****
C               *****************************
...
C               *****************************
C
C**** *locrint*  - Read binary integer field on unit knulre
C
C     Purpose:
C     -------
C     Find string cdfldn on unit knulre and read integer array pfield
C
C**   Interface:
C     ---------
C       *CALL*  *locrint (cdfldn, kfield, kdimax, knulre, kflgre)*
C
C     Input:
C     -----
...
C
      INTEGER (kind=ip_intwp_p) kfield(kdimax)
      CHARACTER*8 cdfldn
C
...
C* Read associated field
      READ (UNIT = knulre, ERR = 210, END = 210) kfield
