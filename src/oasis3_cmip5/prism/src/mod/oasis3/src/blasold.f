      SUBROUTINE blasold (pfild ,kmsize, kfield, pmcoeff, kaux, kaddr,
...
C               *****************************
C
C**** *blasold* - Stupid blas routine
C
C     Purpose:
C     -------
C     Do linear combination of fields with given coefficients
C     before interpolation
C
C**   Interface:
C     ---------
C       *CALL*  *blasold (pfild ,kmsize, kfield, pmcoeff, kaux, kaddr,
...
C
      REAL (kind=ip_realwp_p) pfild(kmsize)
      REAL (kind=ip_realwp_p) paux(ksiztot), pacoeff(kaux)
      INTEGER (kind=ip_intwp_p) kaddr(kaux), kasize(kaux)
C
C* ---------------------------- Local declarations ----------------------
C
      INTEGER (kind=ip_intwp_p) istop
      LOGICAL llchk
C
C* ---------------------------- Poema verses ----------------------------
C
...
C
     ...
     $    ' Do linear combination of fields'
      ...
      istop = 0
C
C* Check all fields have same dimensions as current field
C
      DO 110 jc = 1, kaux
     ...
     $          jc, 2)
        ...
        ENDIF 
 110  CONTINUE 
      IF (istop .GT. 0) CALL HALTE('STOP in blasold')
C
C
