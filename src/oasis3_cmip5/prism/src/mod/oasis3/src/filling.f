...
C
      REAL (kind=ip_realwp_p) pfild(klon, klat), plon(klon, klat), 
     $    plat(klon, klat)
      REAL (kind=ip_realwp_p) pworka(klon, klat),  pworkb(klon, klat) 
      REAL (kind=ip_realwp_p) pworkc(klon, klat)
      INTEGER (kind=ip_intwp_p) kmask(klon,klat), kmesh(klon, klat)
