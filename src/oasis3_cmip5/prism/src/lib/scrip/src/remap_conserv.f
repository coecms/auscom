...
C         calls of SCRIP
C       - Introduction of a logical flag to distinguish between the first
C         and following calls of scrip conservative remapping
C       - Masking of overlapping grid points for source and target grid
C         For these points, links and weights of overlapped point are used
C
...
!-----------------------------------------------------------------------
!
!     CVS:$Id: remap_conserv.f,v 1.3 2006/07/25 13:43:57 valcke Exp $
!
...
! Check overlapping point of the source grid
      ...
      PRINT*, 'Check overlapping point of the source grid'
...
! Check overlapping point of the target grid
      ...
      PRINT*, 'Check overlapping point of the target grid'
...
!bi003      print *,'grid1 sweep '
      write(101,*) 'grid1 sweep '
      do grid1_add = 1,grid1_size
...
!bi003                    print*, 'ERROR=>integration stalled:' 
!bi003                    print*, 'num_subseg exceeded limit'
!bi003                    print*, '=>Verify corners in grids.nc, especially'
!bi003                    print*, 'if calculated by OASIS routine corners'           
                 write(101,*)'ERROR=>integration stalled:'
                 write(101,*)'num_subseg exceeded limit'
                 write(101,*)'=>Verify corners in grids.nc, especially'
                 write(101,*)'if calculated by OASIS routine corners'
                    stop 
...
!bi003      print *,'grid1 end sweep '
      write(101,*)'grid1 end sweep '
...
!bi003      print *,'grid2 sweep '
      write(101,*)'grid2 sweep '
      do grid2_add = 1,grid2_size
...
!bi003      print *,'grid2 end sweep '
      write(101,*)'grid2 end sweep '
...
!bi003      print *, 'Total number of links = ',num_links_map1
       write(101,*)'Total number of links = ',num_links_map1
...
!bi003          print *,'Grid 1 area error: n, area, mask ='
          write(101,*)'Grid 1 area error: n, area, mask ='
     &          ,n,grid1_area(n), grid1_mask(n)
        endif
        if (grid1_centroid_lat(n) < -pih-.01 .or.
     &      grid1_centroid_lat(n) >  pih+.01) then
!bi003            print *,'Grid 1 centroid lat error: n, centroid_lat, mask='
         write(101,*)'Grid 1 centroid lat error: n, centroid_lat, mask='
...
!bi003            print *,'Grid 2 area error:  n, area, mask ='
            write(101,*)'Grid 2 area error:  n, area, mask ='
     &      ,n,grid2_area(n), grid2_mask(n)
        endif
        if (grid2_centroid_lat(n) < -pih-.01 .or.
     &      grid2_centroid_lat(n) >  pih+.01) then
!bi003          print *,'Grid 2 centroid lat error: n, centroid_lat, mask='
         write(101,*)'Grid 2 centroid lat error: n, centroid_lat, mask='
...
!bi003          print *,'Map 1 weight < 0 '
          write(101,*)'Map 1 weight < 0 '
!bi003          PRINT *,
          write(101,*)
...
!bi003          print *,'Map 1 weight > 1 '
          write(101,*)'Map 1 weight > 1 '
!bi003    PRINT *,
          write(101,*)
     &        'grid1_add, grid2_add, wts_map1, grid1_mask, grid2_mask',
     &        grid1_add, grid2_add, wts_map1(1,n), 
...
!bi003          print *,
!       *** this printing takes too much stdio space (which some machines such as XE
!           does not like) when calculating the remapping coefficients in the 1st run!         
           write(101,*)
     &'Error sum wts map1:grid2_add,grid2_centroid_lat,norm_factor='
     &,grid2_add,grid2_centroid_lat(grid2_add),
     &norm_factor,grid2_mask(grid2_add)
        endif
      end do
...
!     &,grid1_add,grid1_centroid_lat(grid1_add),
!     &norm_factor,grid1_mask(grid1_add)
!          endif
!        end do
!      endif
...
!-----------------------------------------------------------------------
     ...
     &                 vec1_lat*vec2_lon - vec2_lat*vec1_lon
               ...
               if (neg/=0 .and. pos/=0) then
                  loutside=.false.
!bi003                  print*, 'The mesh ',srch_add(cell),' is non-convex'
!bi003                  print*, 'srch_corner_lat=',srch_corner_lat(:,cell)
!bi003                  print*, 'srch_corner_lon=',srch_corner_lon(:,cell)
                write(101,*)'The mesh ',srch_add(cell),' is non-convex'
                write(101,*)'srch_corner_lat=',srch_corner_lat(:,cell)
                write(101,*)'srch_corner_lon=',srch_corner_lon(:,cell)
               endif
            endif
            if (loutside) then
              lat2 = beglat
