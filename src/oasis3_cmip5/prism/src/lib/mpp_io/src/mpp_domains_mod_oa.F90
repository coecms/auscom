...
!                Sophie Valcke: renamed the routine with _oa suffix
!
...
module mpp_domains_mod_oa
  use mod_kinds_mpp
!a generalized domain decomposition package for use with mpp_mod_oa
...
#include <os.h>
  ...
  character(len=128), private :: version= &
       '$Id: mpp_domains_mod_oa.F90,v 1.5 2006/05/31 13:22:42 ghattas Exp $'
...
!          write( stdlog(),'(a)' )trim(version_update_domains2D)
      ...
      call mpp_domains_set_stack_size(983040) !default, pretty arbitrary
#ifdef use_shmalloc
...
! get the number of divisions of domain
      ...
      if( ndivs.NE.npes )call mpp_error( 'mpp_domains_mod_oa', &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine mpp_define_domains1D( global_indices, ndivs, domain, pelist, &
                                     flags, halo, extent, maskmap,offset)
...
!rv
      logical ::blocks
      integer :: lastie,il_compute_max_size
!rv
      integer :: halosz
...
!RV
      ...
      if( ndivs.GT.ieg-isg+1 )call mpp_error( WARNING, 'MPP_DEFINE_DOMAINS1D: more divisions requested than rows available.' )
...
!rv Block distribution ?
      blocks=.false.
      if(present(offset)) blocks=.true.
      if(blocks) then
       if((.not.present(maskmap)).and.(.not.present(extent))) &
         call mpp_error(FATAL, 'MPP_DEFINE_DOMAINS1D: you have to define maskmap and extent!')
        if(size(offset).ne.ndivs) &
          call mpp_error( FATAL, 'MPP_DEFINE_DOMAINS1D: offset array size must equal number of domain divisions.' )
      endif
!rv
...
!get compute domain
      if( compute_domain_is_global )then
          ...
          n = 0
!RV
          il_compute_max_size=0
!RV
          do ndiv=0,ndivs-1
             if( PRESENT(extent) )then
!rv build the grid by individual block sizes
               if(blocks) then
                 if(mask(ndiv)) then
                   is=isg+offset(ndiv)
                   ie = is + extent(ndiv) - 1
!                 print*,'MPP_DEFINE_DOMAINS: ',pe,':',ndiv,blocks,is,ie,ieg,offset
                 endif
...
!                 if(lastie.gt.0.and.is.ne.lastie+1) &
!                   call mpp_error( FATAL, 'MPP_DEFINE_DOMAINS: block extents do not span space completely.' )
                 lastie=ie
...
!rv for Arnaud
                 domain%list(ndiv)%compute%begin = 0
                 domain%list(ndiv)%compute%end   = 0
               endif
               if(ndiv.eq.ndivs-1.and.lastie.gt.ieg) &
                   call mpp_error( FATAL, 'MPP_DEFINE_DOMAINS: block extents exceed max. space.' )
             else
             if( ie.LT.is )call mpp_error( FATAL, 'MPP_DEFINE_DOMAINS: domain extents must be positive definite.' )
             domain%list(ndiv)%compute%begin = is
             domain%list(ndiv)%compute%end   = ie
!rr 2005-07-20, Luis Kornblueh, MPIMet
             if( ndiv.GT.0 ) THEN
...
!RV
      if(blocks) then
       call mpp_max(domain%compute%max_size)
      endif
!RV
...
!if yes, if_overlap returns true, and the overlap region is returned in [os,oe]
    ...
    subroutine mpp_define_domains2D( global_indices, layout, domain, pelist, &
                                                     xflags, yflags, &
                                     xhalo, yhalo, xextent, yextent, maskmap,offsetx,offsety, name )
!define 2D data and computational domain on global rectilinear cartesian domain (isg:ieg,jsg:jeg) and assign them to PEs
      ...
      integer, intent(in), optional :: offsetx(0:), offsety(0:)
...
!rv Block distribution ?
      blocks=.false.
      if(present(offsetx).and.present(offsety)) blocks=.true.
      if(present(offsetx).and.(.not.present(offsety))) then
         call mpp_error(FATAL,'MPP_DEFINE_DOMAINS1D: you have to define offsetx AND offsety!')
      endif
      if((.not.present(offsetx)).and.present(offsety)) then
         call mpp_error(FATAL,'MPP_DEFINE_DOMAINS1D: you have to define offsetx AND offsety!')
      endif
      if(blocks) then
       if((.not.present(maskmap)).and.(.not.present(xextent))) &
         call mpp_error(FATAL, 'MPP_DEFINE_DOMAINS1D: you have to define maskmap and xextent!')
       if((.not.present(maskmap)).and.(.not.present(yextent))) &
         call mpp_error(FATAL, 'MPP_DEFINE_DOMAINS1D: you have to define maskmap and yextent!')
       if(size(offsetx).ne.ndivx) &
         call mpp_error(FATAL,'MPP_DEFINE_DOMAINS2D: offsetx array does not match layout!')
       if(size(offsety).ne.ndivy) &
         call mpp_error(FATAL,'MPP_DEFINE_DOMAINS2D: offsety array does not match layout!')
      endif
!rv
...
!do domain decomposition using 1D versions in X and Y
      call mpp_define_domains( global_indices(1:2), ndivx, domain%x, &
           pack(pearray(:,jpos),mask(:,jpos)), xflags, xhalo, xextent, mask(:,jpos),offset=offsetx )
      call mpp_define_domains( global_indices(3:4), ndivy, domain%y, &
           pack(pearray(ipos,:),mask(ipos,:)), yflags, yhalo, yextent, mask(ipos,:),offset=offsety )
      else
!do domain decomposition using 1D versions in X and Y
      call mpp_define_domains( global_indices(1:2), ndivx, domain%x, &
...
!set up domain%list
      ...
      do i = 0,n-1
         ...
         if( pe.EQ.mpp_root_pe() .AND. PRESENT(name) )write( stdlog(), '(2x,i3,1x,4i5,3x,4i5)' )pes(m), domain_info_buf(9:)
      end do
