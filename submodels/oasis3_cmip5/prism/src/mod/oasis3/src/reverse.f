      SUBROUTINE reverse (pfild ,kxlon ,kylat, cdxord, cdyord)
      USE mod_kinds_oasis
C****
C               *****************************
...
C               *****************************
C
C**** *reverse* - Ordering routine
C
C     Purpose:
C     -------
C     Reorder field according to cdxord and cdyord information
C
C**   Interface:
C     ---------
C       *CALL*  *reverse (pfild ,kxlon ,kylat, cdxord, cdyord)*
C
C     Input:
C     -----
...
C                cdxord : latitude ordering
C                cdyord : longitude ordering
C
C     Output:
C     ------
...
C
      IF (nlogprt .GE. 2) THEN
          ...
          WRITE (UNIT = nulou,FMT = *) ' '
          WRITE (UNIT = nulou,FMT = *) 
     $    '           ROUTINE reverse  -  Level 3'
          WRITE (UNIT = nulou,FMT = *) 
     $    '           ***************     *******'
          WRITE (UNIT = nulou,FMT = *) ' '
          WRITE (UNIT = nulou,FMT = *) 
     $    ' Reorder field North ---> South '
          WRITE (UNIT = nulou,FMT = *) ' and from Greenwhich ---> West '
          WRITE (UNIT = nulou,FMT = *) ' if necessary '
          WRITE (UNIT = nulou,FMT = *) ' '
          WRITE (UNIT = nulou,FMT = *) ' '
      ENDIF
