...
!-----------------------------------------------------------------------
!
!     CVS:$Id: remap_bilinear.f,v 1.1.1.1 2003/07/29 17:20:10 valcke Exp $
!
...
!bi003            print *,'Point coords: ',plat,plon
!bi003            print *,'Dest grid lats: ',src_lats
!bi003            print *,'Dest grid lons: ',src_lons
!bi003            print *,'Dest grid addresses: ',src_add
!bi003            print *,'Current i,j : ',iguess, jguess
            write(101,*)'Point coords: ',plat,plon
            write(101,*)'Dest grid lats: ',src_lats
            write(101,*)'Dest grid lons: ',src_lons
            write(101,*)'Dest grid addresses: ',src_add
            write(101,*)'Current i,j : ',iguess, jguess
     ...
     &    'Using non-masked points among 4 nearest neighbors.'
          ...
          do srch_add = min_add,max_add
            distance=acos(coslat_dst*cos(grid1_center_lat(srch_add))*
     ...
     &          (.not. grid1_mask(srch_add) .and. lextrapdone)) then
                    distance=acos(coslat_dst
     &                  *cos(grid1_center_lat(srch_add))*
     &                  (coslon_dst*cos(grid1_center_lon(srch_add)) +
     &                  sinlon_dst*sin(grid1_center_lon(srch_add)))+
     &                  sinlat_dst*sin(grid1_center_lat(srch_add)))
              ...
              WRITE(nulou,*) 
     &        'Nearest non masked neighbour is source point ',src_addnn
