...
! data structure for storing information about individual variables
 type variable_type
   ...
   integer :: ndim, natt, xtype, varid_out, shape(3), axes(NF90_MAX_VAR_DIMS)
   logical :: static, do_missval, do_avg, use_data_wt, skip
   real(8)        :: tavg(3)
   ...
   integer :: istat, ifile, itime, ivar, idim, nvar, ndim, natt, dimlen, varid,      &
              ...
              nvar_out, nv_dimid, nc, nca, ncu
   integer :: max_shape(3), shape(3), tid(0:3), avgid(3), start(4)
...
!-----------------------------------------------------------------------
  ...
  ! create version string (may replace with CVS $Id: time_average.f90,v 1.2 2006/08/10 18:17:24 fms Exp $)
...
!-----------------------------------------------------------------------
 ...
 do ifile = 1, MAX_FILES
    ...
    !--- opening input files ---
     istat = NF90_OPEN (trim(file_names(ifile)), NF90_NOWRITE, ncid_in)
     if (istat /= NF90_NOERR) call error_handler (ncode=istat)

    !--- get global info for this file ---
     istat = NF90_INQUIRE (ncid_in, ndim, nvar, natt, recdim)
     if (istat /= NF90_NOERR) call error_handler (ncode=istat)
...
!print *, trim(Vars(ivar)%name),': use_data_wt,do_missval=',Vars(ivar)%use_data_wt,Vars(ivar)%do_missval
        ...
        !--- array for reading/writing of netcdf fields ---
         allocate (ddata(max_shape(1),max_shape(2),max_shape(3)))
   ...
   !---------------- OUTPUT FILE INITIALIZATION -----------------
     ...
     if (ifile == 1) then
       ! create file
         istat = NF90_CREATE (trim(file_name_out), NF90_CLOBBER, ncid_out)
      ...
      !  use name from first file to get variable id
       ...
       !--- read variable ---
         select case (Vars(ivar)%ndim)
            case(0)
               istat = NF90_GET_VAR (ncid_in, varid, ddata(1,1,1), start(1:1))
               if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
            case(1)
               istat = NF90_GET_VAR (ncid_in, varid, ddata(1:ix,1,1), start(1:2))
               if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
            case(2)
               istat = NF90_GET_VAR (ncid_in, varid, ddata(1:ix,1:jy,1), start(1:3))
               if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
            case(3)
               istat = NF90_GET_VAR (ncid_in, varid, ddata(1:ix,1:jy,1:kz), start(1:4))
               if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
         end select
        ...
        !---------- SUM FIELDS ----------
         ...
         if (Vars(ivar)%use_data_wt) then
           if (Vars(ivar)%do_missval) then
              where (ddata(1:ix,1:jy,1:kz) .ne. Vars(ivar)%missval)
                Vars(ivar)%data_sum = Vars(ivar)%data_sum+tavg(3)*ddata(1:ix,1:jy,1:kz)
           ...
           if (numtime == 1) then
                Vars(ivar)%data_sum = ddata(1:ix,1:jy,1:kz)
           else
                Vars(ivar)%data_sum = Vars(ivar)%data_sum+tavg(3)*ddata(1:ix,1:jy,1:kz)
                Vars(ivar)%data_wt_max = Vars(ivar)%data_wt_max+tavg(3)
           endif
   ...
   do ivar = 1, nvar_out
      ...
      if (Vars(ivar)%use_data_wt) then
          ...
          if (Vars(ivar)%do_missval) then
             where (Vars(ivar)%data_wt > data_wt_min)
                ddata(1:ix,1:jy,1:kz) = Vars(ivar)%data_sum/Vars(ivar)%data_wt
             elsewhere
                ddata(1:ix,1:jy,1:kz) = Vars(ivar)%missval
             endwhere
          else
          ...
          if (numtime == 1) then
             ddata(1:ix,1:jy,1:kz) = Vars(ivar)%data_sum
          else
             ddata(1:ix,1:jy,1:kz) = Vars(ivar)%data_sum/Vars(ivar)%data_wt_max
          endif
      endif
    ...
    ! correct non-float data for truncation error
      if (Vars(ivar)%xtype /= NF90_REAL8 .and. Vars(ivar)%xtype /= NF90_REAL4) then
          if (Vars(ivar)%do_missval) then
              where (ddata(1:ix,1:jy,1:kz) /= Vars(ivar)%missval) &
                ddata(1:ix,1:jy,1:kz) = ddata(1:ix,1:jy,1:kz) + sign(half,ddata(1:ix,1:jy,1:kz))
          else
                ddata(1:ix,1:jy,1:kz) = ddata(1:ix,1:jy,1:kz) + sign(half,ddata(1:ix,1:jy,1:kz))
          endif
      endif
    ...
    !---- write data to output file ----
      select case (Vars(ivar)%ndim)
         case(0)
            istat = NF90_PUT_VAR (ncid_out, Vars(ivar)%varid_out, ddata(1,1,1), start(1:1))
            if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
         case(1)
            istat = NF90_PUT_VAR (ncid_out, Vars(ivar)%varid_out, ddata(1:ix,1,1), start(1:2))
            if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
         case(2)
            istat = NF90_PUT_VAR (ncid_out, Vars(ivar)%varid_out, ddata(1:ix,1:jy,1), start(1:3))
            if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
         case(3)
            istat = NF90_PUT_VAR (ncid_out, Vars(ivar)%varid_out, ddata(1:ix,1:jy,1:kz), start(1:4))
            if (istat /= NF90_NOERR) call error_handler &
                                        ('var= '//trim(Vars(ivar)%name), ncode=istat)
      end select
...
!#######################################################################
 ...
 real    :: fillval
    ...
    ! will not allow fields with more than four dimensions
    if (Var%ndim > 4) call error_handler &
                ('input field ('//trim(Var%name)//') has more than 4 axes')
...
!#######################################################################
 ...
 type(variable_type), intent(inout) :: Var
   ...
   !--- allocate space for data (and data wt when necessary) ---
    allocate (Var%data_sum(shape(1),shape(2),shape(3)))
    if (.not.Var%use_data_wt) then
       if (ntime > 1 .and. Var%do_missval .and. .not.Var%static) then
           allocate (Var%data_wt(shape(1),shape(2),shape(3)))
