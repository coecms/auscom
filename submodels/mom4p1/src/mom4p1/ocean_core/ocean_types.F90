...
!################################################################################################
  ...
  type, public :: ocean_grid_type
     ...
     real, dimension(isd:ied,jsd:jed)    :: gradH  ! sqrt(dht_dx**2+dht_dyx**2) on u-cells (m/m) at time tau
     ...
     real, dimension(isd:ied,jsd:jed,nk)  :: umask       ! land/sea mask for U cells based on s-coordinate
  ...
  type, public :: ocean_domain_type
     ...
     integer :: ioff , joff               ! index offset for absolute indices if MOM4_STATIC_ARRAYS (0 otherwise)
  ...
  type, public :: ocean_prog_tracer_type
     ...
     real, dimension(isd:ied,jsd:jed)      :: tcalving         ! tracer concentration in frozen runoff from land (e.g., calving land ice)
  ...
  type, public :: ocean_velocity_type
     ...
     real, dimension(isd:ied,jsd:jed,2)      :: smf           ! momentum flux per mass into ocean surface (N/m^2)
...
! not MOM4_STATIC_ARRAYS
  ...
  type, public :: ocean_thickness_type
     ...
     real, dimension(:,:,:,:), _ALLOCATABLE :: rho_dzt   _NULL ! rho(kg/m^3)*thickness(m) of T cell at 3 times
     real, dimension(:,:,:),   _ALLOCATABLE :: rho_dztr  _NULL ! 1.0/(rho*dzt) at time taup1
     real, dimension(:,:,:,:), _ALLOCATABLE :: rho_dzu   _NULL ! rho(kg/m^3)*thickness(m) of U cell at 3 times
     real, dimension(:,:,:),   _ALLOCATABLE :: rho_dzur  _NULL ! 1.0/(rho*dzu) at time taup1
     ...
     real, dimension(:,:,:),   _ALLOCATABLE :: dzt     _NULL ! thickness of T cell (m) at time tau/taup1
     real, dimension(:,:,:),   _ALLOCATABLE :: dzu     _NULL ! thickness of U cell (m) at time tau/taup1 
     real, dimension(:,:,:),   _ALLOCATABLE :: dzwt    _NULL ! vert distance (m) between T points at time tau/taup1 
     real, dimension(:,:,:),   _ALLOCATABLE :: dzwu    _NULL ! vert distance (m) between U points at time tau/taup1

     real, dimension(:,:,:),   _ALLOCATABLE :: dztlo _NULL ! distance (m) from T-cell point to top of T-cell 
     real, dimension(:,:,:),   _ALLOCATABLE :: dztup _NULL ! distance (m) from T-cell point to bottom of T-cell 
  ...
  type, public :: ocean_grid_type
     ...
     real,    dimension(:,:), _ALLOCATABLE :: gradH  _NULL ! sqrt(dht_dx**2+dht_dyx**2) on u-cells (m/m) at time tau
  ...
  type, public :: ocean_prog_tracer_type
     ...
     real, _ALLOCATABLE, dimension(:,:)     :: runoff_tracer_flux  _NULL ! tracer flux in liquid runoff (e.g., kg*degC/(m^2 s) for temp)
     real, _ALLOCATABLE, dimension(:,:)     :: calving_tracer_flux _NULL ! tracer flux in solid runoff (e.g., kg*psu/(m^2 s)  for salinity)
  ...
  type, public :: ocean_velocity_type
     ...
     real, _ALLOCATABLE, dimension(:,:,:)     :: smf             _NULL ! momentum flux per mass into ocean surface (N/m^2)
  ...
  type, public :: ocean_external_mode_type
     ...
     real, _ALLOCATABLE, dimension(:,:)     :: patm_for_sea_lev _NULL ! ice plus atmospheric press on tracer cell, passed to coupler (Pa)
