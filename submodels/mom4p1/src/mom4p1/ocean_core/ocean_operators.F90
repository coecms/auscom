...
!
!<DESCRIPTION>
! This module computes discrete operators used by MOM4.  
!</DESCRIPTION>
!
! <INFO>
!
...
! </INFO>
  ...
  character (len=128) :: version = &
       '$Id: ocean_operators.F90,v 16.0.54.1.8.1.32.1 2009/10/10 00:41:58 nnz Exp $'
...
! </DESCRIPTION>
!
! For mom4 with generalized vertical coordinates, ud has an extra
! density factor built in.  
!
function DIV_UD (ud)

    real, intent(in), dimension(isd:,jsd:,:) :: ud
...
! either the free surface height or the bottom pressure. Result is 
! centered onto the U-cell for use in updating barotropic velocity. 
!
! The algorithm is a speedier version of
! <BR/>
...
!
!
function GRAD_BAROTROPIC_P (press)

    real, intent(in), dimension(isd:,jsd:) :: press
    real, dimension(isd:ied,jsd:jed,2)     :: GRAD_BAROTROPIC_P
    integer :: i, j
    ...
    do j=jsc-halo,jec
       do i=isc-halo,iec
          ...
          GRAD_BAROTROPIC_P(i,j,1) = (pip_fay - p_fay)*Grd%dxur(i,j)*Grd%umask(i,j,1)
          GRAD_BAROTROPIC_P(i,j,2) = (pjp_fax - p_fax)*Grd%dyur(i,j)*Grd%umask(i,j,1)
       enddo
    enddo
    GRAD_BAROTROPIC_P(iec+1:,:,:) = 0.0
    GRAD_BAROTROPIC_P(:,jec+1:,:) = 0.0
