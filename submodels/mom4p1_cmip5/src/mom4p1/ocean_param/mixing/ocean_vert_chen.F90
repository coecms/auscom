...
!
!<DESCRIPTION>
! This scheme is commonly used by researchers at the CSIRO 
...
use ocean_parameters_mod,      only: missing_value, rho0r, rho_cp, cp_ocean
...
character(len=256) :: version=&
     '$Id: ocean_vert_chen.F90,v 16.0.42.1.2.1.16.1.20.1 2009/10/10 00:42:31 nnz Exp $'
...
!       dtts   = density time step (sec)                                  <BR/>   
!       dtuv   = internal mode time step (sec)                            <BR/>   
...
! </DESCRIPTION>
!
! Ver PoD subroutine ocean_vert_chen_init (Grid, Domain, Time, Time_steps, T_prog, T_diag)
subroutine ocean_vert_chen_init (Grid, Domain, Time, Time_steps, T_prog, debug)
...
!-----------------------------------------------------------------------
  ...
  id_hbl = register_diag_field('ocean_model','hbl',Grd%tracer_axes_wt(1:2), &
       ...
       missing_value = missing_value, range=(/-1.e5,1.e5/))
  id_diff_cbt_chen_t = register_diag_field('ocean_model','diff_cbt_chen_t',Grd%tracer_axes_wt(1:3), &
       Time%model_time, 'vert diffusivity from chen for temp', 'm^2/sec',&
       missing_value = missing_value, range=(/-1.e5,1.e5/))

  id_diff_cbt_chen_s = register_diag_field('ocean_model','diff_cbt_chen_s',Grd%tracer_axes_wt(1:3), &
...
! Do Richardson no mixing according to whatever law you like
!
! Probably should make this a distinct user supplied/customisable subroutine
!
! Law here is
!
...
! Convection
  ...
  ! default salinity coefficient equal to thermal  (?)
...
!  input
!      
!      real dbloc(ij_bounds)   = local delta buoyancy         (m/s^2)        <BR/> 
...
!      real mixmask(ij_bounds,nk) ! fraction of cell which resides in mixed layer <BR/>  
...
!
  ...
  do j=jsc,jec
    do i=isc,iec
!
! Calulate buoyancy fluxes in mixed layer
!
      ...
      BoS=-(grav*rho0r) * T_prog(index_salt)%field(i,j,1,tau) *pme(i,j) * sbeta(i,j) / (Dens%rho(i,j,1,tau)+epsln)
!
...
! Penetrative SSW contribution
  ...
  do j=jsd,jed
    do i=isd,ied
      if(kbl(i,j) > 0) then
         saltbl(i,j) = T_prog(index_salt)%field(i,j,kbl(i,j),taum1)
...
!
  ...
  do j=jsd,jed
    do i=isd,ied
      saltbl(i,j) = T_prog(index_salt)%field(i,j,kblm1(i,j),taum1)
...
! </DESCRIPTION>
!
subroutine ri_for_chen (Time, Thickness, Velocity, T_prog, pressure_at_depth, rho, rho_taum1)
  ...
  real, dimension(isd:,jsd:,:), intent(in) :: rho, rho_taum1
