...
!  of legacy, as this was done in mom4p0 versions. The default is .false.
!  </DATA> 
...
character(len=128)  :: version='$Id: ocean_shortwave_csiro.F90,v 16.0.108.1.4.1 2009/12/01 23:44:13 smg Exp $'
...
! OFAM changes
logical :: use_sw_pen_legacy      = .false. ! Old sw algorithm. 
...
! </DESCRIPTION>
  ...
  stdoutunit=stdout();stdlogunit=stdlog() 
    ...
    elseif(sw_frac_top==1.0) then 
        ...
        write(stdoutunit,*) &
        ' This is not the standard approach used in mom4.'
    ...
    if(sw_pen_fixed_depths) then
        ...
        write(stdoutunit,*)&
        '             Time varying thicknesses are the norm in mom4p1, so recommend'
        write(stdoutunit,*) &
        '             setting sw_pen_fixed_depths=.false.  However, to reproduce mom4p0'
        write(stdoutunit,*) &
...
subroutine sw_source_csiro (Time, Thickness, T_diag, swflx, index_irr, Temp, sw_frac_zt)
  ...
  if ( use_sw_pen_legacy ) then
     ...
     do k=1,nk-1
        ...
        if(sw_pen_fixed_depths) then 
            ...
            if(Grd%zw(k) <= zmax_pen) then 
                zw_sw(isc:iec,jsc:jec) = Grd%zw(k)
                call sw_pen(zw_sw, sw_fk_zw, k)
                zt_sw(isc:iec,jsc:jec) = Grd%zt(k)
                call sw_pen(zt_sw, sw_fk_zt, k)
            else
                sw_fk_zt(:,:) = 0.0
                sw_fk_zw(:,:) = 0.0
            endif
  ...
  ! note that this irradiance tracer is only used in MOM4 for purpose of 
...
!  hemisphere.
!
! New algorithm. Shortwave is only computed at on level. No need for expensive 
! search. 
!
! </DESCRIPTION>
!
