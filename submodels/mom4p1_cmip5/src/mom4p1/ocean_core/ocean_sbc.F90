...
!  </DATA> 
!
!  <DATA NAME="use_calving_idealized" TYPE="logical">
!  Option to add an idealized calving flux into the ocean.
!  This field is assumed to be time independent. 
!  Default use_calving_idealized=.false.
!  </DATA> 
!
...
integer :: id_calving_idealized    =-1
...
real, dimension(isd:ied,jsd:jed) :: calving_idealized  ! idealized mass flux of calving land ice into ocean
#else
...
real, allocatable, dimension(:,:) :: calving_idealized ! idealized mass flux of calving land ice into ocean
#endif
...
logical :: use_calving_idealized        =.false.
...
! OFAM Namelist variables
logical :: restore_mask_OFAM
logical :: river_temp_OFAM
...
namelist /ocean_sbc_nml/ temp_restore_tscale, salt_restore_tscale, salt_restore_under_ice, salt_restore_as_salt_flux,        &
         ...
         use_calving_idealized, land_model_heat_fluxes, use_full_patm_for_sea_level, max_delta_salinity_restore, do_flux_correction, &
...
subroutine ocean_sbc_init(Grid, Domain, Time, T_prog, T_diag, Velocity, &
  ...
  allocate(calving_idealized(isd:ied,jsd:jed))
#endif
  data              = 0.0
  pme_taum1         = 0.0
  river_taum1       = 0.0
  ...
  if(read_restore_mask) then 
      if(restore_mask_OFAM) then
        id_restore_mask_OFAM=init_external_field('INPUT/restore_mask.nc','restore_mask', domain=Domain%domain2d)
      else
...
!sjm 20101222 move this use_calving_idealized stuff out of the above   "if(read_restore_mask) then" loop

  if(use_calving_idealized) then
      write(stdout(),*) &
      '==>Note from ocean_sbc_mod: applying time independent prescribed calving flux.'
!sjm 20101222 Katja points out no diff for Greenland melt expt     call read_data('INPUT/calving_idealized','calving_idealized',data,Domain%domain2d)
     call read_data('INPUT/calving_idealized.nc','calving_idealized',data,Domain%domain2d)
      do j=jsc,jec
         do i=isc,iec
            calving_idealized(i,j) = data(i,j)*Grd%tmask(i,j,1)
         enddo
      enddo
  endif
  id_calving_idealized = register_static_field('ocean_model','calving_idealized', &
                   Grd%tracer_axes(1:2),'prescribed calving flux', 'kg/(m^2 sec)',&
                   missing_value=missing_value,range=(/-1e20,1e20/))
  if (id_calving_idealized > 0) then
     used = send_data(id_calving_idealized, calving_idealized(:,:), Time%model_time, &
 ...
 id_river = register_diag_field('ocean_model','river', Grd%tracer_axes(1:2),    &
       ...
       '(kg/m^3)*(m/sec)', missing_value=missing_value,range=(/-1e-1,1e-1/))
 ...
 id_evap = register_diag_field('ocean_model','evap', Grd%tracer_axes(1:2),&
       ...
       '(kg/m^3)*(m/sec)', missing_value=missing_value,range=(/-1e-2,1e-2/),&
...
!#ifdef AusCOM
!    Ocean_sfc%gradient(isc_bnd:iec_bnd,jsc_bnd:jec_bnd,:) = 0.0
!#endif
  end where 
...
!#ifdef AusCOM
!  real, dimension(isd:ied,jsd:jed,2) :: sslope
!#endif
...
#ifdef AusCOM
  sslope(:,:,:) = GRAD_BAROTROPIC_P(Thickness%sea_lev(:,:))
#endif
...
subroutine get_ocean_sbc(Time, Ice_ocean_boundary, Thickness, Ext_mode, T_prog, Velocity, &
  ...
  if (use_waterflux) then
      ...
      if(land_model_heat_fluxes) then  
          ...
          ! OFAM sometimes specifies the temperatures of the rivers according to climatological SST.
          call time_interp_external(id_restore(index_temp), Time%model_time, data)
          do j=jsc,jec
             do i=isc,iec
                T_prog(index_temp)%trunoff(i,j)  = max(runoff_temp_min,data(i,j))
...
!           in the course of run. Keeping it can maintain global mean SSL!
      ...
      ! add idealized calving.  Do so AFTER the 
      ! use of zero_net_water_coupler, since we wish to actually add
      ! a net amount of water to the ocean with this option.  
      if(use_calving_idealized) then
        do j=jsc,jec
           do i=isc,iec
             river(i,j) = river(i,j) + calving_idealized(i,j)
           enddo
        enddo
      endif
...
subroutine flux_adjust(Time, T_diag, T_prog, Velocity, river, melt, pme, aice)
...
subroutine flux_adjust(Time, T_diag, T_prog, Velocity, river, melt, pme)
  ...
  if( id_restore_mask_OFAM > 0 ) then
    call time_interp_external(id_restore_mask_OFAM, Time%model_time, restore_mask)
  endif
...
#else
      ! use near-frazil condition or ioaice passed from ice model as a proxy for where sea-ice is present 
      if(.not. salt_restore_under_ice) then
          do j=jsc,jec
             do i=isc,iec
                if(Grd%tmask(i,j,1) == 1.0) then
                    if ( use_ioaice ) then
                      ...
                      ice_present = T_prog(index_temp)%field(i,j,1,tau) <= -0.0539*T_prog(index_salt)%field(i,j,1,tau)
                    endif
                    if( ice_present ) then
                        open_ocean_mask(i,j) = 0.0
                    endif
