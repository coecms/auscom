...
MODULE diag_util_mod
  ...
  CHARACTER(len=128),PRIVATE  :: version =&
       & '$Id: diag_util.F90,v 17.0.4.7 2009/12/10 16:24:12 sdu Exp $'
...
11  FORMAT(a, ' since ', i4.4, '-', i2.2, '-', i2.2, ' ', i2.2, ':', i2.2, ':', i2.2)
    ...
    IF( aux_present .AND. .NOT.match_aux_name ) THEN
       ...
       ! </ERROR>
!RASF       IF ( mpp_pe() == mpp_root_pe() )&
!RASF            & CALL error_mesg ('diag_util opening_file',&
!RASF            &'one axis has auxiliary but the corresponding field'//&
!RASF            &' is NOT found in file '//files(file)%name , WARNING)
    END IF
