...
!#endif
  ...
  character(len=128), private :: version= &
       '$Id: mpp_mod_oa.F90,v 1.5 2006/05/31 13:22:42 ghattas Exp $'
...
!  integer, private, allocatable :: request(:)
  integer, public, allocatable :: mpp_request(:)
#ifdef _CRAYT3E
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine mpp_init( flags,mpp_comm ,logfile)
      integer, optional, intent(in) :: flags,mpp_comm
...
!      integer, optional, intent(in) :: flags, in, out, err, log
      ...
      if(PRESENT(mpp_comm)) then
        comm_intern=mpp_comm
      else
        comm_intern=MPI_COMM_WORLD
      endif
!
      if(present(logfile)) then
        etcfile=trim(logfile)
        configfile=trim(logfile)
        logfile_defined=.true.
      endif
!
      if( .NOT.opened )then
      call MPI_INIT(error)
...
#ifdef use_libMPI
      current_peset_num = 0
      peset(0)%id = comm_intern
      call MPI_COMM_GROUP(  peset(0)%id, peset(0)%group, error )
#endif
...
!rv
!rv Status 'REPLACE' eads to an unpredictable behaviour on SX-6
!rv      if( pe.EQ.root_pe )open( unit=etc_unit, file=trim(etcfile), status='REPLACE' )
!rv
      if( pe.EQ.root_pe )then
          if(logfile_defined) then
            inquire( file=trim(etcfile), opened=opened,number=io_num)
            if(opened) then
              etc_unit=io_num
            else
...
!rv
      if(present(logfile)) then
        ...
        if( pe.NE.root_pe ) then
          open( unit=etc_unit, file=trim(etcfile), status='OLD' )
          close(etc_unit)
        endif
      endif
!            write(0,*)'etc_unit=',etc_unit
!rv
...
!      end if
      if( pe.EQ.root_pe )then
          log_unit = get_unit()
!rv Status 'REPLACE' leads to an unpredictable  behaviour on the SX-6.
!rv          open( unit=log_unit, file=trim(configfile), status='REPLACE' )
        if(logfile_defined) then
          inquire( file=trim(configfile), opened=opened,number=io_num)
          if(opened)then
            log_unit=io_num
          else
            open( unit=log_unit, file=trim(configfile), status='UNKNOWN' )
            close(log_unit)
          endif
        else
          open( unit=log_unit, file=trim(configfile), status='UNKNOWN' )
...
!messages
    ...
    function stdout()
      ...
      if( pe.NE.root_pe ) then
        stdout = etc_unit
        inquire( file=trim(etcfile), opened=opened ,number=tmp_unit)
        if(.not.opened) open(file=etcfile,unit=etc_unit,status='UNKNOWN')
      endif
      if(logfile_defined)then
        ...
        if( pe.EQ.root_pe )then
            ...
            if( opened )then
                ...
                out_unit=tmp_unit
                stdout=out_unit
!              write(0,*) 'stdout:',tmp_unit,etcfile
                call FLUSH(out_unit)

            else
                tmp_unit=get_unit()
!              write(0,*) 'stdout(not):',tmp_unit,etcfile
!ac
                open( unit=tmp_unit, status='UNKNOWN', file=trim(etcfile), err=10 )
!ac
              out_unit=tmp_unit
            end if
      ...
      return
 10 logfile_defined=.false.
    call mpp_error( FATAL, 'STDOUT: unable to open '//trim(etcfile))
    end function stdout
    ...
    function stderr()
      ...
      if(logfile_defined)then
        ...
        if( pe.EQ.root_pe )then
            inquire( file=trim(configfile), opened=opened ,number=tmp_unit)
            if( opened )then
                err_unit=tmp_unit
                stderr=err_unit
                call FLUSH(err_unit)
!              write(0,*) 'stderr:',tmp_unit
            else
!              write(0,*) 'stderr(not):',tmp_unit
                tmp_unit=get_unit()
!ac
                open( unit=tmp_unit, status='UNKNOWN', file=trim(configfile), err=10 )
!ac
              err_unit=tmp_unit
            end if
 ...
 10 logfile_defined=.false.
    call mpp_error( FATAL, 'STDERR: unable to open '//trim(configfile))
    ...
    function stdlog()
      ...
      if( pe.EQ.root_pe )then
	      ...
	      call FLUSH(log_unit)
              ...
              log_unit=get_unit()
!ac
              open( unit=log_unit, status='UNKNOWN', file=trim(configfile), err=10 )
!ac
          end if
          stdlog = log_unit
      else
...
#ifdef use_libMPI
!reiner Let's do the MPI_finalize outside the mpp environment
!reiner      call MPI_FINALIZE(error)
#endif
...
!MPI_COMM_CREATE call. The parent is typically  peset(0)%id, though it could also
...
!count must return a number between 0 and count_max
      ...
      real(DOUBLE_KIND), save       :: count0  ! use to prevent integer overflow
...
#ifdef use_LAM_MPI
#define MPI_TYPE_ MPI_DOUBLE_PRECISION
#else 
...
#endif
#ifdef use_libMPI
         if( mpp_request(i).NE.MPI_REQUEST_NULL )call MPI_WAIT( mpp_request(i), stat, error )
#endif
      end do
      if( current_clock.NE.0 )call increment_current_clock(EVENT_WAIT)
      return
...
!the call to MPI_ABORT is not trapped by TotalView on sgi
              call MPI_ABORT(  peset(0)%id, 1, error )
#endif
#endif
