...
!       3_2-3     S.Valcke       2004/11     Adapted for GSIP
!
!*-----------------------------------------------------------------------
...
!        Arnaud Caubel - Fecit
!        Sophie Valcke -CERFACS (Adapted for GSIP)
!     ----------------------------------------------------------------
...
!     ----------------------------------------------------------------
!
   WRITE(nulprt, *) 'End of clim_define'
   CALL FLUSH(nulprt)
#endif
   RETURN
...
!     ----------------------------------------------------------------
   ...
   INTEGER(kind=ip_intwp_p) :: il_testvar
   REAL(kind=ip_realwp_p)   :: rl_testvar
   REAL(kind=ip_double_p)   :: dl_testvar
   CHARACTER*32  cltest
   INTEGER (kind=ip_intwp_p) :: il_bytesize, il_iosize
   INTEGER (kind=ip_intwp_p) :: il_int_iosize, il_rbyt, il_dbyt
!     ----------------------------------------------------------------
!
...
!
   IF ( ktype.EQ.CLIM_Integer ) THEN
       myport(2,nports) = CLIM_Integer
       myport(3,nports) = il_bytesize
   ...
   ELSEIF ( ktype.EQ.CLIM_Double ) THEN
       myport(2,nports) = CLIM_Double
       INQUIRE (iolength=il_iosize) dl_testvar
       il_dbyt = il_iosize/il_int_iosize*il_bytesize
       myport(3,nports) = il_dbyt
   ELSE
       kinfo = CLIM_BadType
