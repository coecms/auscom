...
C
      ...
      DO 200 ix = 1, kngx1
        DO 210 iy = 1, kngy1 
          IF (p1surf(ix, iy) .eq. 0.) itot = itot + 1
 210    CONTINUE
 200  CONTINUE
      IF (itot .eq. ing1) THEN
...
C
      IF (nlogprt .GE. 2) THEN
          WRITE (UNIT = nulan,FMT = *) ' '
          WRITE (UNIT = nulan,FMT = *) 
     $        ' Ocean average mesh distance = ', sqrt(zamsh1) 
      ENDIF
      IF (itot .ne. ing1) then
C* Printing
C
          IF (nlogprt .GE. 2) THEN  
              WRITE (UNIT = nulan,FMT = *) 
     $            ' ******************************* '
              WRITE (UNIT = nulan,FMT = *) 'Ocean total surface = ', 
     $            zstot1
              WRITE (UNIT = nulan,FMT = *) 
     $            ' Ocean inverse of sum surf. elts. squared = ', zhhi1
          ENDIF
      ENDIF
C
C
...
C
      ...
      DO 300 ix = 1, kngx2
        DO 310 iy = 1, kngy2 
          IF (p2surf(ix, iy) .eq. 0.) itot = itot + 1
 310    CONTINUE
 300  CONTINUE
      IF (itot .eq. ing2) THEN
         WRITE (UNIT = nulan,FMT = *) 'No statistics will be performed'
